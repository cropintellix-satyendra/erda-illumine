...
class OnboardingExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents,WithStyles,WithChunkReading,ShouldQueue
{
    use Exportable;
    ...
    */
    public function collection()
    {
        ...
        $farmer = FarmerPlot::where('status',$request->status )->whereHas('farmer', function($q) use($request) {
                                        $q->where('onboarding_form',1);

                                        if(isset($request->rolename) && $request->rolename == 'Viewer' && !$request->state){
                              ...
                              ->with('farmer')
                              ->when($request,function($a) use($request){
    ...
    //              'Remarks',
    //              'PlotData','Plot Images', 'Plot Unique ID', 'Plot No','Gender
    //              '];
