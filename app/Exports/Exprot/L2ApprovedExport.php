...
class L2ApprovedExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents, WithStyles, WithChunkReading
{
    ...
    public function collection()
    {
        ...
        // dd($request->userid);
        $farmer = FarmerPlot::with('FinalFarmerLandimg')->where('status', 'Approved')->where('final_status', 'Approved')->whereHas('final_farmers', function ($q) use ($request) {
    ...
    public function map($farmer): array
    {
        ...
        // dd($farmer->FinalFarmerLandimg);
        // dd($farmer->final_farmers->UserApprovedRejected->name);
        $payload_fields = [
        ...
        if ($farmer->final_farmers->state_id == 36) {
            ...
            if (isset($request->state)  && $request->state == 29) {
                array_push(
                    ...
                    $farmer->farfinal_farmersmer->longitude ?? "-",
