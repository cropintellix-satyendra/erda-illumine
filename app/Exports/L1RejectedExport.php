...
class L1RejectedExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents,WithStyles,WithChunkReading,ShouldQueue
{
    use Exportable;
    ...
    */
    public function collection()
    {
        ...
        $farmer = FarmerPlot::where('status','Rejected')->whereHas('farmer', function($q) use($request) {
                                        $q->where('onboarding_form',1);
                                        
                                        if(isset($request->rolename)  && $request->rolename != 'SuperAdmin' && $request->rolename != 'Viewer'){
                                            $VendorLocation = VendorLocation::where('user_id',$request->userid)->first();
                                        ...
                                        ->when('filter',function($w) use($request){
                                            if(isset($request->rolename)  && $request->rolename != 'SuperAdmin' && $request->rolename != 'Viewer' && $request->status != 'Pending'){
                                                ...
                                                $w->where('aprv_recj_userid',$request->userid);
                                            }
