   ...
   */
   public function store(Request $request){
    ...
    try{
        ...
        if($farmer){
            FinalFarmer::where('farmer_uniqueId', $request['farmer_uniqueId'])->forceDelete();
      ...
      //this way it will be directly available for crop data and further 
       ...
       return response()->json(['success'=>true,'message'=>'Farmer Store Successfully',
                                'FarmerId'=>$farmer_id, 'FarmerUniqueID'=>$request->farmer_uniqueId],200);
    }catch(\Exception $e){
       return response()->json(['error'=>true,'message'=>'Somethings went wrong']);
    }
   }
   ...
   public function storeLocation(Request $request){
     ...
     try{
        ...
        foreach($Farmer_data as $items){
          ...
          $Farmer->farmer_survey_id = substr($Farmer->district, 0, 2).'/'.substr($Farmer->taluka, 0, 3).'/'.substr($Farmer->village, 0, 3).'/'.$request->farmer_uniqueId;
          $Farmer->save();
        }        
       if(!$Farmer_data){
         return response()->json(['error'=>true,'message'=>'something went wrong'],500);
    ...
    try{
       ...
       if ($request->hasFile('signature')){
        $path = Storage::disk('s3')->put(config('storagesystems.path').'/'.$request->farmer_unique_id.'/'.'DOCUMENTS', $request->signature);
          ...
          FinalFarmer::where('farmer_uniqueId',$request->farmer_unique_id)->where('plot_no',$request->plot_no)->update(['check_carbon_credit'=>'1',
   ...
   public function FarmerPlotImage(Request $request){
      try{
          //to store plot image
            ...
            $path = Storage::disk('s3')->put(config('storagesystems.path').'/'.$request->farmer_unique_id.'/'.$request->farmer_unique_id.'P'.$request->sr.'/'.'ONBOARDPLOT', $request->image); //Storage::putFile('public/'.config('storagesystems.store').'/'.$request->farmer_unique_id,  $request->image);
...
//   }
    ...
    /**
      ...
      public function search_survey_list(){
        $data = FinalFarmer::select('id','farmer_uniqueId','aadhaar','farmer_survey_id','farmer_name','mobile_access','mobile_reln_owner','mobile','gender','guardian_name'
              ,'country_id','country','state_id','state','district_id','district','taluka_id','taluka','panchayat_id','panchayat','village_id','village')
                      ->where('mobile','like','%'.request()->data.'%')
                      ->orWhere('aadhaar','like','%'.request()->data.'%')
