...
class FarmerEditController extends Controller
{
    ...
    public function farmer_registration_edit($unique, $plotno)
    {
        ...
        $farmerplotimg =  DB::table('final_farmer_land_img')->where('farmer_unique_id', $unique)->where('plot_no', $plotno)->select('id', 'farmer_id', 'farmer_unique_id', 'plot_no', 'path')->get();
     ...
     public function farmer_registration_update(Request $request)
    {
        if (!empty($request->survey_no)) {
            $survey_no = FarmerPlot::where('farmer_uniqueId', $request->unique)->where('reason_id', $request->reason_id)->where('plot_no', $request->plotno)
                ->update([
                    'survey_no' => $request->survey_no, 'status' => 'Pending', 'check_update' => '1', 'final_status' => 'Pending', 'finalreject_timestamp' => null, 'reason_id' => null, 'reject_comment' => null, 'reject_timestamp' => null
                ]);

                $final_farmer = FinalFarmer::where('farmer_uniqueId', $request->unique)->where('plot_no', $request->plotno)
                ->update([
                    'final_status' => 'Pending', 
                    'final_status_onboarding' => 'Pending', 
        ...
        if (!empty($request->plot_area)) {
            ...
            $plot_area = FarmerPlot::where('farmer_uniqueId', $request->unique)->where('reason_id', $request->reason_id)->where('plot_no', $request->plotno)
                ->update([
                    'area_in_acers' => $request->plot_area, 'status' => 'Pending', 'final_status' => 'Pending', 'check_update' => '1', 'finalreject_timestamp' => null, 'reason_id' => null, 'reject_comment' => null, 'reject_timestamp' => null
                ]);
        ...
        if (!empty($request->actual_owner_name)) {
            $plot_area = FarmerPlot::where('farmer_uniqueId', $request->unique)->where('reason_id', $request->reason_id)->where('plot_no', $request->plotno)
                ->update([
                    'actual_owner_name' => $request->actual_owner_name, 'status' => 'Pending', 'check_update' => '1', 'final_status' => 'Pending', 'finalreject_timestamp' => null, 'reason_id' => null, 'reject_comment' => null, 'reject_timestamp' => null
                ]);
     ...
     public function farmer_registration_updateimg(Request $request)
    {
        try {
            ...
            $path = Storage::disk('s3')->put(config('storagesystems.path').'/'.$request->unique.'/'.$request->unique.'P'.$img->plot_no.'/'.'ONBOARDPLOT', $request->image); //Storage::putFile('public/'.config('storagesystems.store').'/'.$request->farmer_unique_id,  $request->image);
            ...
            $plot_area = FarmerPlot::where('farmer_uniqueId', $request->unique)->where('plot_no', $request->plotno)
                ->update([
                    'check_update' => '1', 'status' => 'Pending', 'reason_id' => '', 'reject_comment' => '', 'reject_timestamp' => '', 'final_status' => 'Pending', 'finalreject_timestamp' => ''
                ]);
    ...
    public function aeration_img_update(Request $request)
    {
        ...
        $aeration_updt = Aeration::where('farmer_plot_uniqueid', $request->farmer_plot_uniqueid)->where('aeration_no', $request->aeration_no)->where('pipe_no', $request->pipe_no)->update([
            ...
            'time_survey'  => Carbon::now()->toTimeString(),
    ...
    public function pipe_installtion_img_update(Request $request)
    {
        ...
        $pipe_img_update->financial_year =  $request->financial_year;
        $pipe_img_update->season  =  $request->season;
        ...
        $pipe_img_update->status      =  'Approved';
        $pipe_img_update->l2status      =  'Pending';
    ...
    public function farmer_onboarding_image(Request $request)
    {
        // dd("in");
        ...
        try {
            ...
            if($request->reason_id == "12")
            {
                ...
                $plot_area = FarmerPlot::where('farmer_uniqueId', $request->farmer_uniqueId)->where('plot_no', $request->plotno)
                ->update([
                    'check_update' => '1', 'status' => 'Pending', 'reason_id' => '', 'reject_comment' => '', 'reject_timestamp' => '', 'final_status' => 'Pending', 'finalreject_timestamp' => ''
                ]);

                $final_farmer = FinalFarmer::where('farmer_uniqueId', $request->farmer_uniqueId)->where('plot_no', $request->plotno)
                ->update([
                    'final_status' => 'Pending', 
                    'final_status_onboarding' => 'Pending', 
                     'status_onboarding' => 'Pending',
            ...
            $record =  PlotStatusRecord::create([
                'farmer_uniqueId'           => $request->farmer_uniqueId,
                'plot_no'                   => $request->plotno,
                'farmer_plot_uniqueid'      => $request->farmer_uniqueId . 'P' . $request->plotno,
                'level'                     => 'AppUser',
                'status'                    => 'Pending',
                'comment'                   => 'Farmer Image Updated From APP',
                'timestamp'                 => Carbon::now(),
                'user_id'                   => auth()->user()->id,
            ]);
            if (!$farmer) {
            ...
            return response()->json(['success' => true, 'farmerId' => $farmer->farmer_id, 'farmerUniqueId' => $farmer->farmer_uniqueId], 200);

            }

            //aadhar photo Upload
            if($request->reason_id == "13")
            {
            ...
            $record =  PlotStatusRecord::create([
                ...
                'level'                     => 'AppUser',
                'status'                    => 'Pending',
                'comment'                   => 'Aadhar Image Updated From APP',
                'timestamp'                 => Carbon::now(),
                'user_id'                   => auth()->user()->id,
            ]);
            if (!$farmer) {
    ...
    public function updatePipeInstallation(Request $request)
    {
        ...
        $pipe_img = DB::table('polygon_validations')->where('farmer_plot_uniqueid', $request->farmer_plot_uniqueid)->first();
        ...
        $pipe_img_validation  = DB::table('polygon_validations')->insert([
            'polygon_id'                => $pipe_img->polygon_id,
            'farmer_uniqueId'           => $pipe_data->farmer_uniqueId,
            'farmer_plot_uniqueid'      => $request->farmer_plot_uniqueid,
            'plot_no'                   => $pipe_data->plot_no,
