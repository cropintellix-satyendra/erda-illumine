...
class FarmerViewController extends Controller
{
    ...
    //     $approved_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Approved')->where('surveyor_id', auth()->user()->id)->distinct()->count();
    //     $pending_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Pending')->where('surveyor_id', auth()->user()->id)->distinct()->count();

    //     // $reject_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Rejected')->where('surveyor_id', auth()->user()->id)->distinct()->count();

    //     $reject_pipeinstallation = DB::table('pipe_installation_pipeimg as pi')
    //     ->select('pi.farmer_plot_uniqueid')
    //     ->leftJoin('pipe_installation_pipeimg as pi2', function ($join) {
    //         $join->on('pi.farmer_plot_uniqueid', '=', 'pi2.farmer_plot_uniqueid')
    //             ->whereRaw('pi.created_at < pi2.created_at');
    //     })
    //     ->whereNull('pi2.farmer_plot_uniqueid') // Filters out non-latest records
    //     ->where('pi.l2status', 'Rejected')
    //     ->where('pi.surveyor_id', auth()->user()->id)
    //     ->distinct()
    //     ->count();
    ...
    //         'success' => True, 'FarmersApproved' => $FarmersApproved, 'FarmerPending' => $FarmerPending, 'FarmersReject' => $FarmersReject,
    ...
    public function user_form_count()
    {
        $FarmersApproved = DB::table('final_farmers')->where('status_onboarding', 'Approved')->where('onboarding_form', 1)->where('surveyor_id', auth()->user()->id)->count();
        $FarmerPending   = DB::table('final_farmers')->where('status_onboarding', 'Pending')->where('onboarding_form', 1)->where('surveyor_id', auth()->user()->id)->count();
        $FarmersReject   = DB::table('final_farmers')->where('status_onboarding', 'Rejected')->where('onboarding_form', 1)->where('surveyor_id', auth()->user()->id)
        ->whereNotExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('farmer_farm_details')
                ->whereRaw('farmer_farm_details.farmer_uniqueId = final_farmers.farmer_uniqueId')
        ...
        $approved_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Approved')->where('surveyor_id', auth()->user()->id)->distinct()->count();
        $pending_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Pending')->where('surveyor_id', auth()->user()->id)->distinct()->count();
        $reject_pipeinstallation = DB::table('pipe_installation_pipeimg')->select('farmer_plot_uniqueid')->where('l2status', 'Rejected')->where('l2trash',0)->where('surveyor_id', auth()->user()->id)->distinct()->count();
        ...
        $approved_polygon = DB::table('polygons')->where('final_status', 'Approved')->where('surveyor_id', auth()->user()->id)->count();
        $pending_polygon = DB::table('polygons')->where('final_status', 'Pending')->where('surveyor_id', auth()->user()->id)->count();
        $reject_polygon = DB::table('polygons')->where('final_status', 'Rejected')->where('surveyor_id', auth()->user()->id)->count();
    ...
    {
        $farmers = FinalFarmer::where('surveyor_id', auth()->user()->id)->where('onboarding_form', '1')->where('onboarding_form', '1')
            ->select(
                'id',
                'surveyor_id',
                // 'surveyor_name',
    ...
    {
        $farmers = FinalFarmer::where('surveyor_id', auth()->user()->id)->where('farmer_uniqueId', 'like', '%' . request('uniqueid') . '%')->where('onboarding_form', '1')
            ->select(
                'id',
                'surveyor_id',
                // 'surveyor_name',
    ...
    {
        $Farmers =  FinalFarmer::where('surveyor_id', auth()->user()->id)->where('final_status_onboarding', 'Rejected')->where('farmer_uniqueId', $request->farmer_uniqueId)->where('onboarding_form', '1')
            ->select(
                'id',
                'surveyor_id',
                // 'surveyor_name',
            ...
            // ->with(['FarmerPlot:farmer_id,farmer_uniqueId,plot_no,area_in_acers,land_ownership,actual_owner_name,survey_no,status,reject_comment,reject_timestamp,check_update'])->first();
              ...
              // ->with(['FarmerPlot:farmer_plot_uniqueid,farmer_id,farmer_uniqueId,plot_no,area_in_acers,land_ownership,actual_owner_name,survey_no,status,reject_comment,reject_timestamp,check_update','reason'])->first();

              ->with([
                'FarmerPlot' => function ($query) {
                    $query->select(
                        'farmer_plot_uniqueid',
                        'farmer_id',
                        'farmer_uniqueId',
            ...
            $Farmers->district_district = $Farmers->district->district ?? null;
            $Farmers->taluka_taluka = $Farmers->taluka->taluka ?? null;
            $Farmers->panchayat_panchayat = $Farmers->panchayat->panchayat ?? null;
            $Farmers->village_village = $Farmers->village->village ?? null;

        unset($Farmers->state); 
        unset($Farmers->district); 
        unset($Farmers->taluka); 
        unset($Farmers->panchayat); 
        unset($Farmers->village); 


        //    $Farmers->state_name = $Farmers->state->name ?? null;
        //     unset($Farmers->state); // Remove state relationship to prevent circular reference in JSON

            // $Farmers->farmerPlot->transform(function ($plot) {
            //     // Perform any transformation on farmer plot if needed
            //     return $plot;
            // });
    ...
    {
        $farmers = FinalFarmer::where('surveyor_id', auth()->user()->id)->where('status_onboarding', request('status'))->where('onboarding_form', '1')
        ->whereNotExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('farmer_farm_details')
                ->whereRaw('farmer_farm_details.farmer_uniqueId = final_farmers.farmer_uniqueId')
    ...
    public function aeration_list()
    {
        $aeration  = Aeration::where('surveyor_id', auth()->user()->id)->where('l2_status', 'Rejected')
            ->select('id', 'pipe_installation_id', 'farmer_uniqueId', 'farmer_plot_uniqueid', 'plot_no', 'aeration_no', 'pipe_no', 'date_survey', 'time_survey', 'reason_id','season','financial_year')
            ->with(
                'farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id',
    ...
    public function farmer_aeration_list_search()
    {   
        $aeration  = Aeration::where('surveyor_id', auth()->user()->id)
        ->where('farmer_uniqueId', 'like', '%' . request('uniqueid') . '%')
        ->where('l2_status', 'Rejected')
        ...
        ->with(
            'farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id,state_id,district_id,taluka_id,panchayat_id,village_id',
            'reject_reason:id,reasons',
            'farmerapproved.state','farmerapproved.district','farmerapproved.taluka','farmerapproved.panchayat','farmerapproved.village'
        )
        ->orderBy('updated_at', 'desc')
        ->get();

        $aeration->transform(function ($item) {
            $item->state_name = $item->farmerapproved->state->name ?? null;
            // dd($item);
            $item->district_district = $item->farmerapproved->district->district ?? null;
            $item->taluka_taluka = $item->farmerapproved->taluka->taluka ?? null;
            $item->panchayat_panchayat = $item->farmerapproved->panchayat->panchayat ?? null;
            $item->village_village = $item->farmerapproved->village->village ?? null;


            // Unset nested relations to avoid serialization issues
            unset($item->farmerapproved->state);
            unset($item->farmerapproved->district);
            unset($item->farmerapproved->taluka);
            unset($item->farmerapproved->panchayat);
            unset($item->farmerapproved->village);
    ...
    public function pipe_installtion_list()
    {
        $pip_intallation  = PipeInstallationPipeImg::where('surveyor_id', auth()->user()->id)->where('l2status', 'Rejected')->where('l2trash', 0)
            ->select('id', 'farmer_uniqueId', 'farmer_plot_uniqueid', 'plot_no', 'created_at', 'lat', 'lng', 'pipe_no', 'distance', 'reason_id','financial_year','season')
            ->with(
                'farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id,document_no,mobile,state_id,district_id,taluka_id,village_id,area_in_acers,panchayat_id',
                'reject_reason:id,reasons',
                'reject_validation_detail:id,farmer_plot_uniqueid,status,comment',
                'polygon:farmer_uniqueId,farmer_plot_uniqueid',
                'farmerapproved.state','farmerapproved.district','farmerapproved.taluka','farmerapproved.panchayat','farmerapproved.village',
                'season:id,name'
            )
            ->orderBy('updated_at', 'desc')
            ...
            $pip_intallation->transform(function ($item) {
                $item->state_name = $item->farmerapproved->state->name ?? null;
                // dd($item);
                $item->district_district = $item->farmerapproved->district->district ?? null;
                $item->taluka_taluka = $item->farmerapproved->taluka->taluka ?? null;
    ...
    //         ->select('id', 'farmer_uniqueId', 'farmer_plot_uniqueid', 'plot_no', 'created_at', 'reason_id', 'area_in_acers', 'state', 'district', 'taluka', 'village', 'ranges')
    //         ->with(
    //             'farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id,surveyor_name,aadhaar,mobile',
    //             'reject_reason:id,reasons',
    ...
    //         $ranges = json_decode($item->ranges);
    //         if (!empty($ranges) && is_array($ranges) && count($ranges) >= 1) {
    //             $firstLatLongPair = [
    //                 'lat' => $ranges[0]->lat,
    //                 'lng' => $ranges[0]->lng,
    //             ];
    //             $item->firstLatLongPair = $firstLatLongPair;
    //         } else {
    //             $item->firstLatLongPair = null;
    //         }
    //         unset($item->ranges);
    //         $response[] = $item;
    //     }
    ...
    //         return response()->json(['error' => True, 'Message' => 'No data'], 422);
    //     }
    //     return response()->json(['success' => True, 'polygon' => $response], 200);
    // }

    public function polygon_list()
    {      
        // dd(auth()->user()->id);
        $polygon  = Polygon::where('surveyor_id',auth()->user()->id)->where('final_status','Rejected')
                                    ->select('id','farmer_uniqueId','farmer_plot_uniqueid','plot_no','created_at','ranges' ,'latitude','longitude','financial_year','season','delete_polygon')
                                    ->with('farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id,document_no,mobile,state_id,district_id,taluka_id,village_id,panchayat_id,area_in_acers',
                                              'reject_validation_detaill2.reject_reason:id,reasons',
                                            'reject_validation_detaill2:id,farmer_plot_uniqueid,status,comment,reject_reason_id',
                                            'farmerapproved.state','farmerapproved.district','farmerapproved.taluka','farmerapproved.panchayat','farmerapproved.village',
                                            'seasons:id,name')
                                    ->orderBy('updated_at','desc')
                                    ->get();

                                    $polygon->transform(function ($item) {
                                        $item->state_name = $item->farmerapproved->state->name ?? null;
                                        // dd($item);
                                        $item->district_district = $item->farmerapproved->district->district ?? null;
                                        $item->taluka_taluka = $item->farmerapproved->taluka->taluka ?? null;
                                        $item->panchayat_panchayat = $item->farmerapproved->panchayat->panchayat ?? null;
                                        $item->village_village = $item->farmerapproved->village->village ?? null;
                                        $item->reject_reason = $item->reject_validation_detaill2->reject_reason ?? null;
                                        // Unset nested relations to avoid serialization issues
                                        unset($item->farmerapproved->state);
                                        unset($item->farmerapproved->district);
                                        unset($item->farmerapproved->taluka);
                                        unset($item->farmerapproved->panchayat);
                                        unset($item->farmerapproved->village);
                                        unset($item->reject_validation_detaill2->reject_reason);
        ...
        foreach ($polygon as $list) {
            if ($list->delete_polygon == 1) {
                $firstLatLongPair = [
                    'lat' => $list->latitude,
                    'lng' => $list->longitude,
                ];
            } else {
                $ranges = json_decode($list->ranges);
                if (!empty($ranges) && is_array($ranges) && count($ranges) >= 1) {
                    $firstLatLongPair = [
                        'lat' => $ranges[0]->lat,
                        'lng' => $ranges[0]->lng,
                    ...
                    $firstLatLongPair = null;
                }
            }

            $list->firstLatLongPair = $firstLatLongPair;
            unset($list->ranges);
            $response[] = $list;
        }                                                 
        if(!$polygon){
    ...
    {
        $polygon  = Polygon::where('surveyor_id', auth()->user()->id)->where('final_status', 'Pending')
            ->select('id', 'farmer_uniqueId', 'farmer_plot_uniqueid', 'plot_no', 'created_at')
            ->with('farmerapproved:farmer_uniqueId,farmer_plot_uniqueid,farmer_name,surveyor_id,document_no,mobile')
            ->orderBy('updated_at', 'desc')
            ->get();
    ...
    {
        $pipe_intallation  = DB::table('polygons')->where('farmer_plot_uniqueid', $request->farmer_plot_uniqueid)->select('ranges')->first();
        if (!$pipe_intallation) {
            return response()->json(['error' => True, 'Message' => 'No data'], 422);
        }
