...
public function pipe_pending_lists()
  {
    ...
    ->paginate(25);
      ...
      $status = request()->status;
      $others = "0";
  	  return view('l2validator.pipe.pending-plot',compact('plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
 ...
 //   $Polygon =
  ...
  public function polygon_pending_lists()
  {
    ...
    //level 2 validator get pending plot list of pipe function
	  //Plot list
	//   if(request()->ajax()){
  		$plots_paginate = PipeInstallation::with('farmerapproved','pipe_image')->where('l2_status','Pending')->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-2-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                // dd($VendorLocation);
                    $q->whereIn('state_id',explode(',',$VendorLocation->state));
