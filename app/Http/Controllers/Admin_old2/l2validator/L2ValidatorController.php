...
public function plotEdit($id){
   ...
   public function update(Request $request, $id)
   {
       ...
       if($request->type == 'UpdateBasicform'){
           ...
           $Farmer->aadhaar = $request->aadhaar;
           $Farmer->save();
 ...
 {  
    $plots_paginate= FinalFarmer::where('onboard_completed', '!=', "Processing")->with('ApprvFarmerPlot')->where('final_status','Rejected')->when(request(),function($q){
        $q->where('onboarding_form','1');
        if(request('module') == 'CropData'){//when plot has cropdata
            $q->whereHas('PlotCropData');
        }
