	...
	//   if(request()->ajax()){
            ...
            $pipe_sql = "
                SELECT pipe_installations.*,
                    farmer_plot_detail.area_acre_awd, pipe_installations.plot_no,
                    final_farmers.mobile, final_farmers.farmer_name,final_farmers.district_id,final_farmers.taluka_id,final_farmers.panchayat_id,final_farmers.village_id,pipe_installation_pipeimg.l2status
 ...
 //   $Polygon =
  ...
  public function pending_polygon_list()
  {
	...
	//   if(request()->ajax()){
        ...
        $pipe_sql = "
        SELECT pipe_installations.*,
            farmer_plot_detail.area_acre_awd, pipe_installations.l2_status,
        ...
        where pipe_installations.status = 'Approved'
        AND pipe_installations.l2_status = 'Pending'
 ...
 //   $Polygon =
  ...
  public function pipe_reject_lists()
  {
	...
	//   if(request()->ajax()){
        ...
        $pipe_sql = "
            SELECT pipe_installations.*,
                farmer_plot_detail.area_acre_awd,pipe_installation_pipeimg.l2status as l2_status,
                final_farmers.mobile, final_farmers.farmer_name,final_farmers.district_id,final_farmers.taluka_id,final_farmers.panchayat_id,final_farmers.village_id
            FROM pipe_installations 
            JOIN final_farmers ON pipe_installations.farmer_plot_uniqueid = final_farmers.farmer_plot_uniqueid
            JOIN farmer_plot_detail ON pipe_installations.farmer_plot_uniqueid = farmer_plot_detail.farmer_plot_uniqueid 
            JOIN pipe_installation_pipeimg ON pipe_installations.farmer_plot_uniqueid = pipe_installation_pipeimg.farmer_plot_uniqueid             
            AND pipe_installations.status  = 'Approved'
            AND pipe_installations.deleted_at IS NULL
            AND pipe_installation_pipeimg.deleted_at IS NULL
  ...
  public function polygon_approved_lists()
  {
	  ...
	  //Plot list
	//   if(request()->ajax()){
  		$plots_paginate = PipeInstallation::with('farmerapproved','plot_detail')->where('l2_status','Approved')->whereHas('farmerapproved',function($q){
 ...
 //   $Polygon =
  ...
  public function polygon_reject_lists()
  {
    ...
    //level 2 validator get pending plot list of pipe function
	  //Plot list
	//   if(request()->ajax()){
  		$plots_paginate = PipeInstallation::with('farmerapproved','pipe_image')->where('l2_status','Rejected')->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-2-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
