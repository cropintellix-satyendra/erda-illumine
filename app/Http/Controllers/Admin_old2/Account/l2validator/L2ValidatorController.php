...
public function reject_search()
{
  ...
  {
      $farmer_count = FarmerPlot::where('final_status', 'Pending')->where('status', 'Approved')->groupBy('farmer_uniqueId')->whereHas('farmer',function($q){
          if(auth()->user()->hasRole('L-1-Validator')){
              $VendorLocation = VendorLocation::where('user_id',auth()->user()->id)->first();
              $q->whereIn('state_id',explode(',',$VendorLocation->state));
              if(!empty($VendorLocation->district)){
       ...
       $farmers_count_plot = FarmerPlot::where('final_status', 'Pending')->where('status', 'Approved')->whereHas('farmer',function($q){
              $q->where('onboarding_form','1');
              if(auth()->user()->hasRole('L-1-Validator')){
                  $VendorLocation = VendorLocation::where('user_id',auth()->user()->id)->first();
                  $q->whereIn('state_id',explode(',',$VendorLocation->state));
      ...
      $total_plot_area = FinalFarmer::where('final_status_onboarding','Approved')->where('L2_appr_userid',auth()->user()->id)->when(request(),function($q){
...
// dd($allpolygon);
  ...
  public function approved_detail($id){
    if(auth()->user()->cannot('show farmer')) abort(403, 'User does not have the right roles.');
    $plot = FinalFarmer::with('PlotPipeData')->where('farmer_plot_uniqueid',$id)->first();
    ...
    return view('l2validator.approved-plot-detail',compact('plot','page_title','page_description','action','farmerplots','reject_module','farmerbenefitimg','check_pipedata','valicountplotapprv'));
  }
  ...
  public function pending_detail($id){
      ...
      return view('l2validator.pending-plot-detail',compact('plot','page_title','page_description','action','farmerplots','reject_module','valicountplotapprv','finalcountplotapprv','status'));
    }
        ...
        public function final_farmer_status(Request $request, $type,$UniqueId){
          if($type == "finalonboarding"){
            foreach($request->plots as $no){
                $farm_plot = FarmerPlot::where('farmer_uniqueId',$UniqueId)->where('plot_no',$no['PlotNo'])->first();
                if($farm_plot->final_status == 'Approved'){
                    return response()->json(['error'=>true, 'exists'=>true,'message'=>'Aleardy approved'],500);
                }
                $farm_plot_final = FinalFarmer::where('farmer_plot_uniqueid',$UniqueId.'P'.$no['PlotNo'])->first();                
                if($farm_plot_final){
                    return response()->json(['error'=>true, 'exists'=>true,'message'=>'Aleardy approved'],500);
                }
 ...
 $action = 'table_landownerships';
   ...
   public function reject_detail($id){
     ...
     if($plot->farmer->state_id == 36){
         ...
         $page_description = 'Farmer Plot Detail';
         $action = 'table_farmer';
     return view('l2validator.reject-plot-detail',compact('plot','page_title','page_description','action','farmerplots','reject_module','valicountplotapprv','status'));
   }
