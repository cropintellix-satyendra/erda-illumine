...
class L2CropDataValidationController extends Controller
{
  ...
  public function cropdata_pending_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerCropdata::with('farmerapproved')->where('status','Approved')->where('l2_status','Pending')->whereHas('farmerapproved',function($q){
      ...
      $others = "0";
  	  return view('l2validator.cropdata.pending-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function cropdata_approved_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerCropdata::with('farmerapproved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-2-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
      ...
      $status = request()->status;
      $others = "0";
  	  return view('l2validator.cropdata.approved-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
