...
use App\Exports\L1BenefitDataIndividualExport;

class BenefitValidationController extends Controller
{
    ...
    {
        if($status == 'Pending'){
                $Farmers = FarmerBenefit::with('farmerapproved')->where('status',$status)->where('farmer_uniqueId','like','%'.request()->get('query').'%')->limit(10)->whereHas('farmerapproved',function($q) use($status){
                ...
                $Farmers = FarmerBenefit::with('farmerapproved')->where('status',$status)->where('apprv_reject_user_id',auth()->user()->id)->where('farmer_uniqueId','like','%'.request()->get('query').'%')->limit(10)->whereHas('farmerapproved',function($q) use($status){
                    if(auth()->user()->hasRole('L-1-Validator')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
                        if(!empty($VendorLocation->district)){
  ...
  public function benefit_pending_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerBenefit::with('farmerapproved')->where('status','Pending')->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-1-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
      ...
      $others = "0";
  	  return view('l1validator.benefit.pending-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function benefit_pending_detail($plotuniqueid){
    ...
    $validation_list = BenefitDataValidation::where('farmer_uniqueId',$plotuniqueid)->where('level','L-1-Validator')->get();
    ...
    return view('l1validator.benefit.pending-plot-benefit',compact('plot','page_title','page_description','action','farmerplots',
  ...
  public function cropdata_reject_lists()
  {
     //level 1 validator get pending plot list of pipe function
	  ...
	  if(request()->ajax()){
        ...
        $status = request()->status;
        $others = "0";
      return view('l1validator.benefit.reject-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  //   $Polygon =
     ...
     return view('l1validator.benefit.reject-plot-awd',compact('plot','PipeInstallation','PipesLocation','Polygon','page_title','page_description','action','farmerplots',
  ...
  public function benefit_approved_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerBenefit::with('farmerapproved')->where('status','Approved')->where('apprv_reject_user_id',auth()->user()->id)->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-1-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
      ...
      $status = request()->status;
      $others = "0";
  	  return view('l1validator.benefit.approved-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function benefit_approved_detail($plotuniqueid){
    ...
    return view('l1validator.benefit.approved-benefit-detail',compact('plot','page_title','page_description','action','farmerplots',
                'farmerbenefitimg','updated_polygon','seasons','benefit_data_detail','benefit_data','check_pipedata','validation_list'));
  }
    ...
    public function downloadFile(){
        ...
        if(request('file')){
         if(request('type') == 'BenefitData'){
            $filename = 'L1_BenefitData_'.request('status').'_'.Carbon::now().'.xlsx';


            // return Excel::download(new L1BenefitExport('All',request()), $filename);
            $payload=[
                    'uuid'=>\Str::uuid(),
                    'data'=>[
      ...
      public function excel_download($type, $unique_id, $status){
        $filename = 'L1Benefit_'.$unique_id.'_'.Carbon::now().'.xlsx';
        return Excel::download(new L1BenefitDataIndividualExport($type, $unique_id, $status), $filename);
     }
}
