...
class L2CropDataValidationController extends Controller
{
  ...
  public function cropdata_pending_lists()
  {
    //level 1 validator get pending plot list of pipe function
      ...
      $others = "0";
  	  return view('admin.l2validator.cropdata.pending-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
...
$pagination = new LengthAwarePaginator(
    ...
    ['path' => request()->url(), 'query' => request()->query()]
      ...
      $status = request()->status;
      $others = "0";
  	  return view('admin.l2validator.cropdata.approved-plot',compact('pagination','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
...
//    $plot = FarmerPlot::where('farmer_plot_uniqueid',$plotuniqueid)->first();
   ...
   if($plot->state_id == 36){
     ...
     'reject_module','farmerbenefitimg','updated_polygon','cropdata','check_pipedata','seasons','CropvarietyList','crop_data_detail','validation_list','rolename'));
  }
