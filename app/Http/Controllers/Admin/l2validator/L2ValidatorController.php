...
// use App\Http\Controllers\Admin\l2validator\User;
...
public function reject_search()
{
  ...
  {
    $farmer_count = FarmerCropdata::count();
         $approved = FarmerCropdata::where('l2_status','Approved')->count();
         $pendings = FarmerCropdata::where('l2_status','Pending')->count();
         $rejected = FarmerCropdata::where('l2_status','Rejected')->count();
      ...
      $total_plot_area = number_format((float) $total_plot_area, 2);
      $others = "0";
      return response()->json(['success'=>true,'farmer_count'=>$farmer_count ,'plot_count'=>$farmer_count,
                                  'approved'=>$approved,'pendings'=>$pendings,'rejected'=>$rejected,'totalarea'=>$total_plot_area,'others'=>$others],200);
  }


  public function aeration_counting(Request $request){

    $farmer_count = Aeration::count();
    $approved = Aeration::where('l2_status','Approved')->count();
    $pendings = Aeration::where('l2_status','Pending')->count();
    $rejected = Aeration::where('l2_status','Rejected')->count();
 ...
 $total_plot_area = number_format((float) $total_plot_area, 2);
 $others = "0";
 return response()->json(['success'=>true,'plot_count'=>$farmer_count,
                             'approved'=>$approved,'pendings'=>$pendings,'rejected'=>$rejected,'totalarea'=>$total_plot_area,'others'=>$others],200);

  }

        public function farmer_benefit(Request $request){

            $farmer_count = FarmerBenefit::count();
            $approved = FarmerBenefit::where('l2_status','Approved')->count();
            $pendings = FarmerBenefit::where('l2_status','Pending')->count();
            $rejected = FarmerBenefit::where('l2_status','Rejected')->count();
        ...
        $total_plot_area = number_format((float) $total_plot_area, 2);
        $others = "0";
        return response()->json(['success'=>true,'farmer_count'=>$farmer_count ,'plotcount'=>$farmer_count,
                                    'approved'=>$approved,'pendings'=>$pendings,'rejected'=>$rejected,'totalarea'=>$total_plot_area,'others'=>$others],200);

        }


  // farmer onboarding for L2 validator.
public function approved_lists(Request $request)
{
  ...
  //   if(request()->ajax()){
    ...
    // $plots_paginate=  FinalFarmer::where('onboard_completed', '!=', "Processing")->where('final_status','Approved')->paginate(10);
    // dd($plots_paginate);

        $plots_paginate = FinalFarmer::where('onboard_completed', '!=', "Processing")->with('ApprvFarmerPlot')->where('final_status','Approved')->whereHas('ApprvFarmerPlot',function($q){
  ...
  //   }//end layoutout plot WITH AJAX
     // Onload below code excute first. And after successful load then again ajax make request to above code
        $page_title = 'Farmer onbaording | Approved list';
        $page_description = 'Onboarding | Approved list';
        $action = 'table_farmer';
    ...
    $others = "0";
      return view('l2validator.approved-plot',compact('links','plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
}
...
// dd($allpolygon);
  ...
  public function approved_detail($id){
    //   dd("in");
      ...
      return view('l2validator.approved-plot-detail',compact('plot','page_title','farmer','page_description','action','farmerplots','reject_module','farmerbenefitimg','check_pipedata','status_timeline','cropdata','plot_areas_sum'));
    }
   ...
   * @return \Illuminate\Http\Response
   */
  // Farmer onboarding details for L2
//   public function approved_detail($id){
//     if(auth()->user()->cannot('show farmer')) abort(403, 'User does not have the right roles.');
//     $plot = FinalFarmer::with('PlotPipeData','ApprvFarmerPlot')->where('farmer_plot_uniqueid',$id)->first();
//     // dd($plot);
   ...
   public function pending_lists()
   {
    // dd('bcdh');
     ...
     if(auth()->user()->cannot('farmer')) abort(403, 'User does not have the right roles.');
       //Plot list
     //   if(request()->ajax()){
         $plots_paginate= FinalFarmer::where('onboard_completed', '!=', "Processing")->with('ApprvFarmerPlot')->where('final_status','Pending')->whereHas('ApprvFarmerPlot',function($q){
     ...
     //   })->get();
       ...
       $status = request()->status;
       $others = "0";
         return view('l2validator.pending-plot',compact('links', 'plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
   }


   public function not_eligible_farmer()
 {  
    // dd("in");

    $plots_paginate = FinalFarmer::with('farmer_farm_details')
    ->whereHas('farmer_farm_details', function ($query) {
...
//   })->get();
   ...
   $onboarding_executive  = DB::table('farmers')->where('onboarding_form','1')->groupBy('farmers.surveyor_name')
    ...
    return view('l2validator.non_awd.not-eligible-reject-plot',compact(
        'links',
        'plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
 }
...
//   }
  ...
  public function pending_detail($id,$farmer_uniqueid){
        
    //    dd("in");
          if(auth()->user()->cannot('show farmer')) abort(403, 'User does not have the right roles.');

           $plot = FinalFarmer::with(['PlotPipeData','ApprvFarmerPlot'])->where('farmer_plot_uniqueid',$farmer_uniqueid)->first();
        // dd($farmer_uniqueid);
        $farmerplots =  FinalFarmer::where('farmer_plot_uniqueid',$plot->farmer_plot_uniqueid)->get();
        $farmerplots_area =  FinalFarmer::where('farmer_uniqueId',$plot->farmer_uniqueId)->get();
        // dd($farmerplots_area);
        ...
        $cropdata = FarmerCropdata::with('PlotCropDetails','farmerapproved')->where('farmer_plot_uniqueid',$plot->farmer_plot_uniqueid)->where('plot_no',$plot->plot_no)->get();
        $check_pipedata = DB::table('pipe_installations')->where('farmer_plot_uniqueid',$farmer_uniqueid)->first();
        $farmer = Farmer::where('farmer_uniqueId',$plot->farmer_uniqueId)->first();
        $reject_module = RejectModule::where('type','Onboarding')->where('status',1)->get();
        $page_title = 'Farmer pending Data';
        $page_description = 'Peniding Farmer Plot Detail';
        $action = 'table_farmer';
        $guntha = 0.025000;
        if($plot->state_id == 36){
                $area = number_format((float)$plot->area_in_acers, 2, '.', '');
          ...
          $status_timeline = PlotStatusRecord::with('UserApprovedRejected','Reasons')->where('farmer_plot_uniqueid',$farmer_uniqueid)->get();

          return view('l2validator.pending-plot-detail',compact('plot','page_title','farmer','page_description','action','farmerplots','reject_module','farmerbenefitimg','check_pipedata','status_timeline','cropdata','plot_areas_sum'));
        }
...
public function plotEdit($id){
    ...
    $farmerplots =  FarmerPlot::where('farmer_uniqueId',$plot->farmer_uniqueId)->get();
   //  dd($farmerplots);
    $reject_module = RejectModule::where('type','PipeInstallation')->get();
    $page_title = 'Onboarding | Pending Edit';
    $page_description = 'Farmer Plot Detail';
    ...
    return view('l2validator.pending-plot-edit',compact('plot','page_title','page_description','action','farmerplots','reject_module',
                                'Relationshipowner','states','districts','talukas','panchayats','villages',
                                'minimumvalues',
                               'Farmerplotsimages'));
  }
   /**
   ...
   public function update(Request $request, $id)
   {
       ...
       if($request->type == 'UpdateBasicform'){
           ...
           $Farmer = FinalFarmer::where('farmer_uniqueId', $id)->first();
           ...
           $Farmer->document_no = $request->aadhaar;
           $Farmer->save();
           ...
           try{
               ...
               foreach($request->ownername as $value){
                  ...
                  if($plot_data->land_ownership == 'Own'){
                      $update_name = FinalFarmer::where('id',$plot_data->farmer_id)->update(['farmer_name'=>$value['actual_owner_name']]);
                    //  foreach($plot_data as $data){
                    //  if($data->land_ownership == 'Own'){
        ...
        // dd(request());
           ...
           try{
               ...
               $Farmer = FinalFarmer::where('id',$plot_data->farmer_id)->first();
              ...
              // Fetch Panchayat and Village objects
              $Panchayat = DB::table('panchayats')->where('id', $request->panchayat)->first();
              $village = DB::table('villages')->where('id', $request->village)->first();

              // Update Farmer record with IDs only
              if ($Panchayat) {
                  $Farmer->panchayat_id = $Panchayat->id;
              }
 ...
 {  
    $plots_paginate= FinalFarmer::where('onboard_completed', '!=', "Processing")
    ...
    ->with('ApprvFarmerPlot')->where('final_status','Rejected')->when(request(),function($q){
        $q->where('onboarding_form','1');
        if(request('module') == 'CropData'){//when plot has cropdata
            $q->whereHas('PlotCropData');
        }
...
//   })->get();
   ...
   $onboarding_executive  = DB::table('farmers')->where('onboarding_form','1')->groupBy('farmers.surveyor_name')
    ...
    return view('l2validator.reject-plot',compact(
        'links',
        'plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
 }
    ...
    //     //Plot list
    //   //   if(request()->ajax()){
    //         $plots_paginate = FinalFarmer::where('onboard_completed', '!=', "Processing")->with('ApprvFarmerPlot')->where('final_status','Rejected')->whereHas('ApprvFarmerPlot',function($q){
    //           if(auth()->user()->hasRole('L-2-Validator')){
    //               $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
    //               $q->whereIn('state_id',explode(',',$VendorLocation->state));
    //               if(!empty($VendorLocation->district)){
    ...
    //     $status = request()->status;
    //     $others = "0";
    //       return view('l2validator.reject-plot',compact('links','plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
    // }
...
//    }
    ...
    */
    ...
    public function reject_show($id,$farmer_uniqueid)
    {
     ...
     $farmerplots =  FinalFarmer::where('farmer_plot_uniqueid',$farmer_uniqueid)->get();
     $farmerplots_area =  FinalFarmer::where('farmer_uniqueId',$farmer_uniqueid)->get();
     // dd($farmerplots_area);
     $plot_areas_sum = 0; // Initialize the sum variable
     foreach ($farmerplots_area as $plot_area) {
         $plot_area_value = floatval($plot_area->plot_area);
     ...
     $cropdata = FarmerCropdata::with('PlotCropDetails','farmerapproved')->where('farmer_plot_uniqueid',$plot->farmer_plot_uniqueid)->where('plot_no',$plot->plot_no)->get();
     $check_pipedata = DB::table('pipe_installations')->where('farmer_plot_uniqueid',$id)->first();
     $farmer = FinalFarmer::where('farmer_uniqueId',$plot->farmer_uniqueId)->first();
     $reject_module = RejectModule::all();
     $page_title = 'Approved Farmer\'s Plot';
     $page_description = 'Approved Farmer Plot Detail';
     $action = 'table_farmer';
     ...
     return view('l2validator.reject-show',compact('plot','page_title','farmer','page_description','action','farmerplots','reject_module','farmerbenefitimg','check_pipedata','status_timeline','cropdata','plot_areas_sum'));
