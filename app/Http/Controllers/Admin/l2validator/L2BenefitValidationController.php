...
class L2BenefitValidationController extends Controller
{
  ...
  public function benefit_pending_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	//   if(request()->ajax()){
  		$plots_paginate = FarmerBenefit::with('farmerapproved')->where('status','Approved')->where('l2_status','Pending')->whereHas('farmerapproved',function($q){
	...
	//   }//end layoutout plot WITH AJAX
      ...
      $others = "0";
  	  return view('l2validator.benefit.pending-plot',compact('links','plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
...
// dd($cropdata);
  ...
  public function cropdata_reject_lists()
  {
     //level 1 validator get pending plot list of pipe function
	  //Plot list
	//   if(request()->ajax()){
        $plots_paginate = FarmerBenefit::with('farmerapproved')->where('status','Rejected')->whereHas('farmerapproved',function($q){
        ...
        $status = request()->status;
        $others = "0";
      return view('l2validator.benefit.reject-plot',compact('plots_paginate','links','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function benefit_approved_lists()
  {
	  //Plot list
	//   if(request()->ajax()){
  		$plots_paginate = FarmerBenefit::with('farmerapproved')->where('status','Approved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-1-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
	...
	//   }//end layoutout plot WITH AJAX
      ...
      $status = request()->status;
      $others = "0";
  	  return view('l2validator.benefit.approved-plot',compact('links','plots_paginate','page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
