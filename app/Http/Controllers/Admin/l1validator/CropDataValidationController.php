...
use DateTime;
use App\Exports\CropDataIndividualExport;

class CropDataValidationController extends Controller
{
    ...
    {
        if($status == 'Pending'){
                $Farmers = FarmerCropdata::with('farmerapproved')->where('status',$status)->where('farmer_uniqueId','like','%'.request()->get('query').'%')->limit(10)->whereHas('farmerapproved',function($q) use($status){
                ...
                $Farmers = FarmerCropdata::with('farmerapproved')->where('status',$status)->where('apprv_reject_user_id',auth()->user()->id)->where('farmer_uniqueId','like','%'.request()->get('query').'%')->limit(10)->whereHas('farmerapproved',function($q) use($status){
                    if(auth()->user()->hasRole('L-1-Validator')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
                        if(!empty($VendorLocation->district)){
  ...
  public function cropdata_pending_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerCropdata::with('farmerapproved')->where('status','Pending')->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-1-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
      ...
      $others = "0";
  	  return view('l1validator.cropdata.pending-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function cropdata_pending_detail($plotuniqueid,$plot_no){
    ...
    $crop_data = FarmerCropdata::where('farmer_plot_uniqueid',$plotuniqueid)->get();
    ...
    $validation_list = CropDataValidation::where('farmer_plot_uniqueid',$plotuniqueid)->where('level','L-1-Validator')->get();
    ...
    return view('l1validator.cropdata.pending-plot-crop',compact('plot','page_title','page_description','action','farmerplots',
                'reject_module','farmerbenefitimg','updated_polygon','crop_data','check_pipedata','seasons','CropvarietyList','date_setting','crop_data_detail','validation_list'));
  }
  ...
  public function cropdata_approved_lists()
  {
    //level 1 validator get pending plot list of pipe function
	  //Plot list
	  if(request()->ajax()){
  		$plots = FarmerCropdata::with('farmerapproved')->where('status','Approved')->where('apprv_reject_user_id',auth()->user()->id)->whereHas('farmerapproved',function($q){
            if(auth()->user()->hasRole('L-1-Validator')){
                $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                $q->whereIn('state_id',explode(',',$VendorLocation->state));
                if(!empty($VendorLocation->district)){
      ...
      $status = request()->status;
      $others = "0";
  	  return view('l1validator.cropdata.approved-plot',compact('page_title','page_description','action','seasons','states', 'districts','talukas','panchayats','villages','onboarding_executive','status','others'));
  }
  ...
  public function cropdata_approved_detail($plotuniqueid,$plot_no){
   ...
   $validation_list = CropDataValidation::where('farmer_plot_uniqueid',$plotuniqueid)->where('status','Approved')->where('level','L-1-Validator')->get();
   ...
   if($plot->state_id == 36){
     ...
     return view('l1validator.cropdata.approved-cropdata-detail',compact('plot','page_title','page_description','action','farmerplots',
    ...
    public function downloadFile(){
        ...
        if(request('file')){
         if(request('type') == 'CropData'){
            $filename = 'CropData_'.request('status').'_'.Carbon::now().'.xlsx';
      ...
      public function excel_download($type, $unique_id, $plot_no, $status){
        ...
        return Excel::download(new CropDataIndividualExport($type, $unique_id, $plot_no, $status), $filename);
     }
