...
<div class="container-fluid">
  ...
  <div id="countBoxRow" class="row  ">
                    ...
                    <div class=" bg-info countBox">
                        <div class="card-body mini-stat-img boxm">
                            <div class="text-white">
                                <h2 class="mb-4s text-center t-font text-white" id="TotalPlot">0</h2>
                    ...
                    <table id="example3" class="table table-bordered"  style="min-width: 845px"> 
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($value->final_status == 'Approved')
                                                        <a class="text-success" title="Show">{{$value->farmer_plot_uniqueid}}</a>
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    @if($value->final_status == 'Approved')
                                                     <a class="btn text-success" href="{{ url('admin/farmer/approved/plot')}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                    @endif     
                                                    @if($value->final_status == 'Pending')
                                                    <a class="btn text-info" href="{{ url('admin/farmer/approved/plot')}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                   @endif   
                                                   @if($value->final_status == 'Rejected')
                                                   <a class="btn text-info" href="{{ url('admin/farmer/approved/plot')}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                  @endif                                            
                                                ...
                                                <td>{{ $value->plot_area ??"0.00"}}</td>
                                                <td>{{ $value->mobile }}</td>
                                                <td>{{ $value->state->name ??"NA"}}</td>
                                                <td>{{ $value->district->district ??"NA"}}</td>
                                                <td>{{ $value->taluka->taluka??"NA"}}</td>
                                                <td>{{ $value->village->village ??"NA"}}</td>
                        ...
                        <div class="pagination">
                            @foreach($links as $key => $value)
                                @php
                                    $pageno = explode('page=', $value);
                                    $page = $pageno[1];
                                @endphp
                        
                                <a href="{{ $value }}" class="{{ $page == $current_page ? 'active' : '' }}">
                                    {{ $page }}
                                </a>
                            @endforeach
                        </div>
...
$(function(){
    ...
    //     "pageLength": 10,
    //     "ajax": {
    //         "url": "{!! url('admin/farmers/all-plot') !!}",
    //         "type": "GET",
    ...
    //         { "data": "final_status_onboarding","name":"final_status_onboarding","defaultContent":""},
    ...
    //                         status_color='text-danger';
    //                     }
    //                     // return '<a class="btn '+status_color+'" href="{{ url('admin/farmers/all-farmer/show')}}/'+row.id+'/'+row.farmer_uniqueId+'" title="Show">'+row.farmer_uniqueId+'</a>';
    //                     return '<span class="btn '+status_color+'">'+row.farmer_uniqueId+'</span>';
    //                 @endif
    //             },
    //             "targets": 0,
    //         },
    ...
    //                     status_color='text-danger';
    //                 }
    //                 return '<a class="ss btn '+status_color+'" href="{{ url('admin/farmer/plot')}}/'+row.id+'" title="Show">'+data+'</a>';
    //             @else
    //                 var status_color='text-info';
    //                 if(row.status == 'Approved'){
    //                     status_color='text-success';
    ...
    //                     status_color='text-danger';
    //                 }
    //                 return '<a class="btn '+status_color+'" href="{{ url('admin/farmer/all-farmer/plot')}}/'+row.id+'" title="Show">'+data+'</a>';
    //             @endif
    //             },
    //             "targets": 2,
    //         },
    //         {render: function (data, type, row, meta) {
    //                 return row.apprv_farmer_plot.area_acre_awd;
    //             },
    //             "targets": 3,
    //         },
    //         {render: function (data, type, row, meta) {
    ...
    //                     +' <a class="btn btn-sm btn-success waves-effect waves-light" href="{{ url('admin/farmer/all-farmer/plot')}}/'+row.id+'" data-toggle="tooltip" title="Show"><i class="ti-eye"></i></a>'
    //                    +'</div>';
    ...
    //   });
      ...
      $('.download-excel').on('click',function(e){
              ...
              var parms=$.param(clean({start_date,end_date,state,district,taluka,panchayats,village,farmer_status,executive_onboarding,file,type,status,record,rolename,userid}));
          ...
          $.ajax({
          type:'get',
          url: "{{url('admin/fetch/all-farmer/counting')}}",
          dataType: 'Json',
          success: function(data) {
              document.getElementById("FarmerCount").innerHTML = data.farmercount;
              document.getElementById("TotalPlot").innerHTML = data.total_plot; 
