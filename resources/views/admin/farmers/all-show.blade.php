    ...
    if(auth()->user()->hasRole('L-1-Validator')){
        ...
        $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->orderBy('id','asc')->whereHas('farmer',function($q){
          ...
          return $q;
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('admin/farmers/all-farmer/show',[$prev->id,$prev->farmer_uniqueId])}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('admin/farmers/all-farmer/show',[$next->id,$next->farmer_uniqueId])}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                    ...
                    <div class="card">
                        ...
                        <div class="card-footer p-0">
                            <div class="row">
                                ...
                                @can('Download Excel')
                                <div class="col-6">
                                    <a class="btn text-success btn-sm" href="{{url('admin/download/file/'.'Multiple/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.$plot->status)}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                </div>
                                @endcan
                            </div>
                        </div>
