        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
              if(auth()->user()->hasRole('L-1-Validator')){
                      $prev=\App\Models\FarmerPlot::with('farmer')->where('id','<',$plot->id)->where('status',$status)->orderBy('id','desc')->whereHas('farmer',function($q){
                      ...
                      $next=\App\Models\FarmerPlot::with('farmer')->where('id','>',$plot->id)->where('status',$status)->orderBy('id','asc')->whereHas('farmer',function($q){
                          $q->where('onboarding_form','1');
                              $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                              $q->whereIn('state_id',explode(',',$VendorLocation->state));
                              if(!empty($VendorLocation->district)){
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l2/pending/plot/detail/').'/'.$prev->id}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l2/pending/plot/detail/').'/'.$next->id}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l2/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Pending')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                      ...
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            <div class="row mb-3">
                              ...
                              <a style="width: 31%;" class="btn btn-status m-b-0 mr-3"><span class="btn-txt">Polygon</span></a>
                            </div>
                            <div class="row mb-3">
                            ...
                            <div class="row mb-3">
                                    <a style="width: 30%;" href="{{url('l2/plot/edit/'.$plot->id.'/'.$plot->farmer_uniqueId)}}"
                                      class="btn btn-info m-b-0 mr-3 EditBtn {{ $plot->final_status == 'Approved' ? 'd-none' : ''}}"
                                      
                                    >EDIT</a>
                                <!-- end button end -->
                              <button style="width: 26%;"
                                  data-toggle="modal" data-target="#FinalApproveModal"
                                  class="btn btn-success FinalApproveBtn m-b-0 mr-3" {{ $plot->final_status == 'Approved' ? 'disabled' : ''}}
                                    >
                                    {{ $plot->final_status == 'Approved'? 'Approved' :'Approve' }}
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark"
                                    class="btn btn-danger FinalRejectBtn m-b-0 mr-3" {{ $plot->final_status == 'Rejected' ? 'disabled' : ''}} {{ $plot->final_status == 'Approved' ? 'disabled' : ''}}>
            ...
            <div class="modal-body" style="margin-top: -53px;">
                ...
                {{--  @php dd($plotlist); @endphp --}}
                  <div class="row">
                    ...
                    <div class="col">
                        @if($plotlist->final_status == 'Approved')
                            <h5>L-2 validator</h5>
              ...
              <div class="container">
                <div class="row">
                  ...
                  @foreach($farmerplots as $plotdata)
                          ...
                          <div class="row">
                                <div class="col mt-2">
                                 @if($plotdata->final_status == 'Rejected')
                                        <h5>L-2 validator</h5>
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/pending/search/'.$plot->status) !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/pending/plot/detail/')}}/'+data.id+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$(".FinalSubmitApproval").click(function() {
    ...
    Swal.fire({
		  ...
		  if (result.value == 1) {
                $.ajax({
                  type:'post',
                  url:"{{url('l2/final/status')}}/"+'finalonboarding/{{$plot->farmer->farmer_uniqueId}}',
                  ...
                  error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    if(data.exists){
                        // this will execute when exits is true
...
$(".FinalFarmerReject").click(function() {
   ...
   if(!$('#Finalplotno' + plotno).is(":checked")){
     ...
     if (result.value == 1) {
               $.ajax({
                 type:'post',
                 url:"{{url('l2/final/status/')}}/"+'finalreject'+'/'+'{{$plot->farmer->farmer_uniqueId}}',
