...
function initMap() {
	...
	if(polygon.length>0){
		polygon = polygon.map(function(v,i){
			if(v && typeof v.lat !== 'undefined' && typeof v.lng !== 'undefined'){
 ...
 //   to display stored polygon in pipeinstallation table
      ...
      if(updated_polygon && Array.isArray(updated_polygon) && updated_polygon.length > 0){//if this has updated polygon
                    ...
                    if(v && typeof v === 'object'){
                        $.each(v,function(z,poly){
                            if(poly && typeof poly === 'string'){
                                try{
                                    var poly_color = color[i] || '#FF0000';
                                    var updatepaths= JSON.parse(poly);
                                    if(Array.isArray(updatepaths)){
                                        updatepath=updatepaths.map(function(n,l){
                                            if(n && typeof n.lat !== 'undefined' && typeof n.lng !== 'undefined'){
                                    ...
                                    console.warn('Error parsing polygon data:', poly, e);
                                }
                            }
                        });
                    }
       ...
       if(polygon_bynearby && Array.isArray(polygon_bynearby)){
    ...
    //If the co-ordinates are same that i sthe reason y i used this calculation to show case the Difference
    if (pipe_location && Array.isArray(pipe_location) && pipe_location.length > 0) {
    console.log(pipe_location); // Debugging: Log the locations to ensure multiple entries are present
    for (var i = 0; i < pipe_location.length; i++) {
        // Validate lat/lng before using
        if(pipe_location[i] && typeof pipe_location[i].lat !== 'undefined' && typeof pipe_location[i].lng !== 'undefined'){
            var lat = parseFloat(pipe_location[i].lat);
            var lng = parseFloat(pipe_location[i].lng);

            // Check if lat/lng are valid numbers
            if(!isNaN(lat) && !isNaN(lng) && isFinite(lat) && isFinite(lng)){
                // Apply a small offset to the latitude and longitude
                var offsetLat = lat + (Math.random() - 0.5) * 0.0001;
                var offsetLng = lng + (Math.random() - 0.5) * 0.0001;

                var marker = new google.maps.Marker({
                    position: { lat: offsetLat, lng: offsetLng },
                    map: map, // Ensure the same map instance is used here
                    title: 'Pipe No: ' + pipe_location[i].pipe_no,
                    icon: {
                        url: "https://maps.google.com/mapfiles/ms/icons/yellow-dot.png"
                    }
