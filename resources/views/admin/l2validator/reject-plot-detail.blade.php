...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                  $prev=\App\Models\FarmerPlot::with('farmer')->where('finalreject_userid',auth()->user()->id)->where('id','<',$plot->id)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','desc')->whereHas('farmer',function($q){
                      $q->where('onboarding_form','1');
                    return $q;
                  })->first();
                  $next=\App\Models\FarmerPlot::with('farmer')->where('finalreject_userid',auth()->user()->id)->where('id','>',$plot->id)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','asc')->whereHas('farmer',function($q){
                      $q->where('onboarding_form','1');
                    return $q;
                  })->first();
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l2/reject/plot/detail').'/'.$prev->id}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l2/reject/plot/detail').'/'.$next->id}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l2/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Rejected')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row mb-3">
                                    <a style="width: 30%;" href="{{url('l2/plot/edit/'.$plot->id.'/'.$plot->farmer_uniqueId)}}"
                                      ...
                                      {{ $plot->final_status == 'Rejected' ? 'disabled' : ''}}
                                    >EDIT</a>
                                <!-- end button end -->
                              ...
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark"
                                    class="btn btn-danger FinalRejectBtn m-b-0 mr-3" {{ $plot->final_status != 'Rejected' ? 'disabled' : ''}}>
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/reject/search/'.$plot->status) !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/reject/plot/detail/')}}/'+data.id+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
