    ...
    <div class="row page-titles mx-0">
        <div class="col-sm-3 p-md-0">
            <div class="welcome-text">
                <h4>Farmer Plot Details 5846</h4>
            </div>
        </div>
        <div class="col-md-3">
        ...
        @php
            $prev=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Pending')->where('id','<',$awd_data->id)->orderBy('id','desc')
            ...
            $next=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Pending')->where('id','>',$awd_data->id)->orderBy('id','asc')
            ->when(request(),function($q){
                if(auth()->user()->hasRole('Viewer')){
                                $VendorLocation = DB::table('viewer_locations')->where('user_id',auth()->user()->id)->first();
                                $q->whereIn('state_id',explode(',',$VendorLocation->state));
        ...
        @endphp
        @if($prev)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/pending/aeration/plot/'.$prev->farmer_plot_uniqueid.'/'.$prev->aeration_no.'/'.$prev->pipe_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
        @endif
        @if($next)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/pending/aeration/plot/'.$next->farmer_plot_uniqueid.'/'.$next->aeration_no.'/'.$next->pipe_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
        @endif  
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l2/download/aeration/'.'aeration'.'/'.$plot->farmer_plot_uniqueid.'/'.$awd_data->plot_no.'/'.'Pending/'.$awd_data->aeration_no)}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                  ...
                  <div class="card" style="margin-right: -24px;margin-left: -24px;">
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            ...
                            @if(!Auth::user()->hasRole('Viewer') && !Auth::user()->hasRole('SuperAdmin'))
                            <div class="row">
                              ...
                              <button style="width: 26%;"
                                  ...
                                  {{$awd_data->l2_status == 'Rejected' ? 'Disabled' : ''}}{{$awd_data->l2_status == 'Approved' ? 'Disabled' : ''}} >
                                    Approve
                                    <i class="fa fa-spinner fa-spin Aspinner d-none"></i>
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark" 
                                    class="btn btn-danger RejectBtn m-b-0 mr-3" {{$awd_data->l2_status == 'Approved' ? 'Disabled' : ''}}>
                                    Reject
                              </button>
                            </div>
                            @endif
                                ...
                                <div class="tab-content">
                                    <div id="plot-{{$PipeInstallation->plot_no}}" class="tab-pane active pt-2">
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    <tr><td>Level</td> <td>Aeration no</td> <td>Pipe no</td> <td>Status</td>  <td>Comment</td> <td>L1-Validate</td></tr>
                                                    @foreach($validation_list  as $list)                                                      
                                                       <tr><td>{{$list->level}}</td> <td>{{ $list->aeration_no}}</td><td>{{ $list->pipe_no}}</td> <td>{{ $list->status}}</td>  <td>{{ $list->comment}}</td>  <td>{{ $list->ValidatorUserDetail->name??""}}</td></tr>
                                                    @endforeach                                              
                                                </tbody>
                                            </table>
                                        </div>
                ...
                <div class="container">
                  ...
                  <div class="row">
                    <div class="col mt-2">
                      ...
                      <input type="hidden" value="{{$items->pipe_no}}" id="pipe_no{{$items->aeration_no}}" name="pipe_no"> 
                ...
                @foreach($awd as $items)
                          <div class="row">
                            <div class="col">
                                ...
                                <label for="aeration_no" style="margin-right: 11px;">Pipe no {{$items->pipe_no}}</label>&nbsp;
                                  <input type="checkbox"  id="aeration_no{{$items->aeration_no}}" 
                                  name="aeration_no" value="{{$items->aeration_no }}">&nbsp; 
                                  <input type="hidden" value="{{$items->pipe_no}}" id="pipe_no{{$items->aeration_no}}" name="pipe_no">                                    
                            </div>
                            <div class="col">
                                 <select id="reasons{{$items->aeration_no}}" {{$items->l2_status == 'Rejected' ? 'Disabled' : ""}}
                            ...
                            <div style="margin: 0px 0px 0px 17px;">
                                ...
                                <textarea class="form-control" id="reject_comment{{$items->aeration_no}}"
                                           name="reject_comment" rows="3" cols="50" {{$items->l2_status == 'Rejected' ? 'Disabled' : ""}}></textarea>
                          ...
                          <div class="row">
                                ...
                                <div class="col">
                                  <button {{ $items->l2_status == 'Rejected' ? 'disabled' : ''}} {{ $awd_data->l2_status =='Approved' ? 'Disabled' :'' }}
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get("{!! url('') !!}"+'/{{$rolename}}/view/l2/aeration/search/'+'{{$awd_data->l2_status}}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    ...
                    return '<div><a href="{{ url("/")}}/'+rolename+'/view/l2/pending/aeration/plot/'+data.farmer_uniqueId+'/'+data.aeration_no+'/'+data.pipe_no+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$(".SubmitApproval").click(function() {
    ...
    $.each($("input[name='aeration_no']:checked"), function(){
        ...
        var pipe_no = $('#pipe_no'+$(this).val()).val();
        var ApproveComment  = $('#approve_comment'+$(this).val()).val();
        aeration_no.push({'aeration_no' : $(this).val(), "pipe_no":pipe_no,'ApproveComment' :ApproveComment});
    });
    Swal.fire({
          title: 'Are you sure?',
          text: "You won't be able to revert this!",
          ...
          if (result.value == 1) {
                $.ajax({
                  type:'post',
                  url:"{{url('l2/aeration/status/')}}/"+'approve/{{$plot->farmer_plot_uniqueid}}',
                  data: {_token:'{{csrf_token()}}',method:'post',aeration_no:aeration_no,pipe_no:'{{$awd_data->pipe_no}}'},
                  success:function(data){
...
$(".AerationReject").click(function() {
    ...
    var pipe_no = $('#pipe_no'+aeration_no).val();
    ...
    if(!$('#aeration_no' + aeration_no).is(":checked")){
        ...
        Swal.fire({
          ...
          if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('l2/aeration/status/')}}/"+'reject'+'/'+'{{$plot->farmer_plot_uniqueid}}',
                  data: {_token:'{{csrf_token()}}',method:'post',aeration_no:aeration_no,reasons:reasons,rejectcomment:rejectcomment,pipe_no:pipe_no},
                  success:function(data){
                      $('#Rspinner'+aeration_no).addClass('d-none');
                    location.reload();
