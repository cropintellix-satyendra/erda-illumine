...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        @php
            $prev=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Approved')->where('id','<',$awd_data->id)->orderBy('id','desc')
            ...
            $next=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Approved')->where('id','>',$awd_data->id)->orderBy('id','asc')
            ->when(request(),function($q){
                if(auth()->user()->hasRole('Viewer')){
                                $VendorLocation = DB::table('viewer_locations')->where('user_id',auth()->user()->id)->first();
                                $q->whereIn('state_id',explode(',',$VendorLocation->state));
        ...
        @endphp
        @if($prev)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/approved/aeration/plot/'.$prev->farmer_plot_uniqueid.'/'.$prev->aeration_no.'/'.$prev->pipe_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
        @endif
        @if($next)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/approved/aeration/plot/'.$next->farmer_plot_uniqueid.'/'.$next->aeration_no.'/'.$next->pipe_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
        @endif  
        </div>
    </div>
    <!-- row -->
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    ...
                    return '<div><a href="{{ url("/")}}/'+rolename+'/view/l2/approved/aeration/plot/'+data.farmer_uniqueId+'/'+data.aeration_no+'/'+data.pipe_no+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }                        
            }
        });
