...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
            $prev=\App\Models\FinalFarmer::select('id','farmer_plot_uniqueid')->where('onboarding_form','1')->where('id','<',$plot->id)->where('farmer_uniqueId','<',$plot->farmer_uniqueId)->orderBy('id','desc')->when(request(),function($q){return $q;})->first()??'';
            $next=\App\Models\FinalFarmer::select('id','farmer_plot_uniqueid')->where('onboarding_form','1')->where('id','>',$plot->id)->where('farmer_uniqueId','>',$plot->farmer_uniqueId)->orderBy('id','asc')->when(request(),function($q){return $q;})->first()??'';
          @endphp
          @if($prev)
