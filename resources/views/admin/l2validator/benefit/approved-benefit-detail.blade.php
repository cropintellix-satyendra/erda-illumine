...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
              $prev=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Approved')->where('l2_status','Approved')->where('id','<',$benefit_data_detail->id)->orderBy('id','desc')
              ...
              $next=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Approved')->where('l2_status','Approved')->where('id','>',$benefit_data_detail->id)->orderBy('id','asc')
              ->when(request(),function($q){
                if(auth()->user()->hasRole('Viewer')){
                                $VendorLocation = DB::table('viewer_locations')->where('user_id',auth()->user()->id)->first();
                                $q->whereIn('state_id',explode(',',$VendorLocation->state));
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/approved/benefit/plot/'.$prev->farmer_uniqueId}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>          
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/approved/benefit/plot/'.$next->farmer_uniqueId}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
          @endif
        </div>
    </div>
    <!-- row -->
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    ...
                    return '<div><a href="{{ url("/")}}/'+rolename+'/view/l2/approved/benefit/plot/'+data.farmer_uniqueId+'"><strong>' + data.farmer_uniqueId	 + '</strong> - ' + data.status + '</a></div>';
                }                         
            }
        });
