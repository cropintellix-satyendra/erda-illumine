                    ...
                    <div class=" bg-info countBox">
                        <div class="card-body mini-stat-img boxm">
                            <div class="text-white">
                                ...
                                <h6 class="text-uppercase-mb-3 t-font font-size-16 text-white text-center">Total Plots
                                </h6>
                            </div>
                        </div>
                    </div>
                ...
                <div class="dropdown ml-auto">
                    @can('Download Excel')
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item download-excel" data-type="BenefitData" href="{{url('l2/benefit/download')}}">Benefit</a>
                        </div>--}}
                    @endcan                  
                </div>
              </div>
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="table table-bordered"  style="min-width: 845px"> 
                            ...
                            <tbody>
                                    @if(!empty($pagination) && $pagination->count())
                                        @foreach($pagination as $key => $value)
                                            <tr>
                                                <td>{{$loop->iteration}}</td>
                                                <td>
                                                    @if($value->l2_status == 'Approved')
                                                        <a class="text-success" title="Show">{{$value->farmer_uniqueId}}</a>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    @if($value->l2_status == 'Approved')
                                                     <a class="btn text-success" href="{{ url('')}}/{{$rolename}}/view/l2/approved/benefit/plot/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->benefit }}</a>
                                                     @endif                                                   
                                                </td>
                                                <td>{{ $value->date_survey }}</td>
                                                <td>{{ $value->mobile }}</td>
                                                {{-- <td>{{ $value->state }}</td>
...
$(function(){
    ...
    //         { "data": "date_survey","name":"date_survey"},
    ...
    //                   }
    //                   return '<a class="'+status_color+'" title="Show">'+row.farmer_uniqueId+'</a>';
    ...
    //                     status_color='text-danger';
    //                 }
    //                 return '<a class="btn '+status_color+'" href="{{ url("")}}/'+rolename+'/view/l2/approved/benefit/plot/'+row.farmer_uniqueId+'" title="Show">'+data+'</a>';
    //             },
    //             "targets": 2,
    //         },
    //         {render: function (data, type, row, meta) {
    ...
    //   });
      ...
      $('.download-excel').on('click',function(e){
              ...
              var parms=$.param(clean({start_date,end_date,season,state,district,taluka,panchayats,village,farmer_status,executive_onboarding,file,type,status,rolename,userid}));
              // if(type=='' || type==null){
          ...
          $.ajax({
          type:'get',
          url: "{{url('l2/fetch/appoved/farner_benfit/counting')}}",
          dataType: 'Json',
          success: function(data) {
            console.log(data);
