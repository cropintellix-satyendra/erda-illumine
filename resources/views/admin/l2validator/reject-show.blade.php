    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                ...
                $prev=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','<',$plots->farmer_uniqueId)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','desc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                  return $q;
                })->first();
                $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$plots->farmer_uniqueId)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','asc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                  return $q;
                })->first();
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l2/pending/show').'/'.$prev->id.'/'.$prev->farmer_uniqueId}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l2/pending/show').'/'.$next->id.'/'.$next->farmer_uniqueId}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>Farmer Unique Id</td><td>{{$plots->farmer_uniqueId}}</td>
                                            </tr>
                                            <tr>
                                                <td>Farmer Name</td><td>{{$plots->farmer_name}}</td>
                                            </tr>
                                            <tr>
                                                <td>Mobile Access</td><td>{{$plots->mobile_access}}</td>
                                            </tr>
                                            <tr>
                                                <td>Relationship owner</td><td>{{$plots->mobile_reln_owner}}</td>
                                            </tr>
                                            <tr>
                                                <td>Mobile</td><td>{{$plots->mobile}}</td>
                                            </tr>
                                            <tr>
                                                <td>Plot No.</td><td>{{$plots->no_of_plots}}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        <thead class="thead-primary">
                                            <tr>
                                                <th colspan="{{$plots->state_id == 36 ? '6' : '5'}}" class="text-center">Plot Info</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                          <tr>
                                              <td>Total Plots</td>
                                              <td colspan="{{$plots->state_id == 36 ? '3' : '2'}}">
                                                 {{-- $finalcountplotapprv ? $finalcountplotapprv : '' --}}
                                                 {{ $valicountplotapprv ? $valicountplotapprv : $plots->no_of_plots }} {{-- $plots->no_of_plots --}}
                                              </td>
                                              <td>Area of Plots (Acers)</td>
                                              <td>{{ $plots->total_area_acres_of_guntha ? $plots->total_area_acres_of_guntha : $plots->total_plot_area}}</td>
                                          </tr>
                                            <tr>
                                                <td>Plot No.</td>
                                            ...
                                            @foreach($farmerplots as $plot)
                                                ...
                                                <td>
                                                    ...
                                                    @forelse($Farmerplotsimages as $items)
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    @can('carbon download')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('admin/farmers/download/'.$plots->farmer_uniqueId.'/'.'CARBON'.'/'.'0')}}"><i class="fa fa-download" aria-hidden="true"></i> Carbon Consent</a>
                                    </div>
                                    @endcan
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('admin/download/file'.'/?type=onboarding&file=excel&unique='.$plots->farmer_uniqueId)}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>State</td><td>{{$plots->state}}</td>
                                            </tr>
                                            <tr>
                                                <td>District</td><td>{{$plots->district}}</td>
                                            </tr>
                                            <tr>
                                                <td>Taluka</td><td>{{$plots->taluka}}</td>
                                            </tr>
                                            <tr>
                                                <td>Panchayat</td><td>{{$plots->panchayat}}</td>
                                            </tr>
                                            <tr>
                                                <td>Village</td><td>{{$plots->village}}</td>
                                            </tr>
                                            <tr>
                                                <td>Latitude</td><td>{{$plots->latitude}}</td>
                                            </tr>
                                            <tr>
                                                <td>Logitude</td><td>{{$plots->longitude}}</td>
                                            </tr>
                                            <tr>
                                                <td class="align-top">Remarks</td><td>{{$plots->remarks}}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                @if(Auth::user()->hasRole('SuperAdmin'))
                                                    <a  target="_blank"  href="{{Route('admin.users.edit',$plots->surveyor_id)}}">{{$plots->surveyor_name}}</a>
                                                @else
                                                    {{$plots->surveyor_name}}
                                                @endif
                                                </td>
                                            ...
                                            <tr>
                                                <td>Mobile No</td><td>{{$plots->surveyor_mobile}}</td>
                                            </tr>
                                            <tr>
                                                <td>Email ID</td><td>{{$plots->surveyor_email}}</td>
                                            </tr>
                                            <tr>
                                                <td>Date of Survey</td><td>{{$plots->date_survey}}</td>
                                            </tr>
                                            <tr>
                                                <td>Time of Survey</td><td>{{ $plots->time_survey }} </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                ...
                <div class="col-12">
                  <div class="card" style="margin-right: -24px;margin-left: -24px;">
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            <div class="row mb-3">
                              <a style="width: 30%;" class="active btn btn-status{{$plots->onboarding_form ? '-done' : ''}} m-b-0 mr-3"><span class="btn-txt">Farmer Onboarding</span></a>
                              <a style="width: 26%;" class="CropDataShow btn btn-status{{$plots->cropdata_form ? '-done' : ''}} m-b-0 mr-3"><span class="btn-txt">Crop data</span></a>
                              <a style="width: 31%;" class="btn btn-status{{$plots->pipes_form ? '-done' : ''}} m-b-0 mr-3"><span class="btn-txt">Pipes Installations</span></a>
                            </div>
                            <div class="row mb-3">
                              <a style="width: 30%;" class="btn btn-status{{$plots->awd_form ? '-done' : ''}} m-b-0 mr-3"><span class="btn-txt">AWD Captured</span></a>
                              <a style="width: 26%;" class="FarmerBenefits btn btn-status{{$plots->benefit_form ? '-done' : ''}} m-b-0 mr-3"><span class="btn-txt">Benefits</span></a>
                              <a style="width: 30%;" class="btn btn-status{{$plots->other_form ? '-done' : ''}} m-b-0"><span class="btn-txt">Others</span></a>
                            </div>
                            <!-- end final approve module -->
                            @if(Auth::user()->hasRole('L-2-Validator'))
                            <div class="row mb-3">
                                    <a style="width: 30%;" href="{{url('admin/farmers/plot/edit/'.$plots->id.'/'.$plots->farmer_uniqueId)}}"
                                      class="btn btn-info m-b-0 mr-3 EditBtn d-none"
                                       @if($plots->status_onboarding == 'Approved')
                                        disabled
                                      @elseif($plots->status_onboarding == 'Rejected')
                                        disabled
                                      @else
                                      @endif
                                    >EDIT</a>
                              ...
                              <button style="width: 26%;"
                                  ...
                                  class="btn btn-success FinalApproveBtn m-b-0 mr-3 d-none" {{-- below code is to disable button if --}}
                                    >
                                    {{ $plots->final_status == 'Approved'? 'Approved' :'Approve' }}
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark"
                                    class="btn btn-danger FinalRejectBtn m-b-0 mr-3 d-none">
                                    {{ $plots->final_status == 'Rejected'? 'Rejected' :'Reject' }}
                              </button>
                            </div>
                            @endif
                            <!-- end final approve module -->
                          ...
                          <div class="row">
                            <div class="col">
                                <label for="plotno" style="margin-right: 11px;">Plot no {{$plotdata->plot_no}}</label>&nbsp;
                                  <input type="checkbox"  id="plotno{{$plots->plot_no}}"
                                    {{ $plotdata->status == 'Rejected' ? 'checked disabled' : ''}}
                                    {{ $plotdata->status == 'Approved' ? 'disabled' : ''}}
                                  name="plotno" value="{{$plots->plot_no}}" {{$plots->status_onboarding == 'Approved' ?' ':''}}>&nbsp;
                            ...
                            <div style="margin: 0px 0px 0px 17px;">
                                ...
                                <textarea {{ $plotdata->status == 'Rejected' ? 'readonly' : ''}}  {{ $plotdata->status == 'Approved' ? 'disabled' : ''}}
                                          class="form-control" id="reject_comment{{$plots->plot_no}}"
                                           name="reject_comment" rows="3" cols="50">{{$plots->reject_comment}}</textarea>
                            </div><br>&nbsp;
                          </div>

                          <div class="row">
                                <div class="col mt-2">
                                  {{$plotdata->UserApprovedRejected->name??''}}   /  {{ Carbon\Carbon::parse($plots->reject_timestamp)->toDayDateTimeString() }}<br>
                                  @if(Auth::user()->hasRole('SuperAdmin'))
            ...
            <div class="modal-body" style="margin-top: -53px;">
                <div class="container">
                  ...
                  <div class="row">
                    ...
                    <div class="col">
                       {{$plotlist->UserApprovedRejected->name??''}}   / {{ Carbon\Carbon::parse($plots->appr_timestamp)->toDayDateTimeString() }} <br>
                          @if(Auth::user()->hasRole('SuperAdmin'))
            ...
            <div class="modal-body" style="margin-top: -53px;">
                ...
                {{--  @php dd($plotlist); @endphp --}}
                  <div class="row">
                    ...
                    <div class="col">
                       {{$plotlist->FinalUserApprovedRejected->name ??' '}} {{ $plots->finalaprv_timestamp ? ' / '.Carbon\Carbon::parse($plots->finalaprv_timestamp)->toDayDateTimeString() :'' }} <br>
                          @if(Auth::user()->hasRole('SuperAdmin'))
                ...
                <div class="row">
                  ...
                  @foreach($farmerplots as $plotdata)
                          <div class="row">
                            <div class="col">
                                <label for="plotno" style="margin-right: 11px;">Plot no {{$plotdata->plot_no}}</label>&nbsp;
                                  <input type="checkbox"  id="Finalplotno{{$plots->plot_no}}"
                                    {{ $plotdata->final_status == 'Rejected' ? 'checked disabled' : ''}}
                                    {{ $plotdata->final_status == 'Approved' ? 'disabled' : ''}}
                                  name="Finalplotno" value="{{$plots->plot_no}}" {{$plots->final_status_onboarding == 'Approved' ?' ':''}}>&nbsp;
                            ...
                            <div style="margin: 0px 0px 0px 17px;">
                                ...
                                <textarea {{ $plotdata->final_status == 'Rejected' ? 'readonly' : ''}}  {{ $plotdata->final_status == 'Approved' ? 'disabled' : ''}}
                                          class="form-control" id="Finalreject_comment{{$plots->plot_no}}"
                                           name="Finalreject_comment" rows="3" cols="50">{{$plots->reject_comment}}</textarea>
                            </div><br>&nbsp;
                          </div>

                          <div class="row">
                                <div class="col mt-2">
                                  {{$plotdata->FinalUserApprovedRejected->name??''}} {{ $plots->finalreject_timestamp ? ' / '.Carbon\Carbon::parse($plots->finalreject_timestamp)->toDayDateTimeString() : '' }}<br>
                                  @if(Auth::user()->hasRole('SuperAdmin'))
    ...
    //search
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get("{!! url('l2/reject/search/') !!}", { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/reject/show')}}/'+data.id+'/'+data.farmer_uniqueId+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$(".SubmitApproval").click(function() {
    ...
    Swal.fire({
		  ...
		  if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('admin/farmers/status/')}}/"+'onboarding/{{$plots->farmer_uniqueId}}',
                  data: {_token:'{{csrf_token()}}',method:'post',plots:plots,TotalPlot:"{{$plots->no_of_plots}}"},
    ...
    console.log('to url');
        Swal.fire({
		  ...
		  if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('admin/farmers/status/')}}/"+'reject'+'/'+'{{$plots->farmer_uniqueId}}',
...
$(".DeleteBtn").click(function() {
    ...
    Swal.fire({
		  ...
		  if (result.value == 1) {
                $.ajax({
                  type:'post',
                  url:"{{url('admin/farmers/final/status/')}}/"+'finalonboarding/{{$plots->farmer_uniqueId}}',
                  data: {_token:'{{csrf_token()}}',method:'post',plots:plots,TotalPlot:"{{$plots->no_of_plots}}"},
                  success:function(data){
...
$(".FinalFarmerReject").click(function() {
   ...
   if(!$('#Finalplotno' + plotno).is(":checked")){
     ...
     if (result.value == 1) {
               $.ajax({
                 type:'post',
                 url:"{{url('admin/farmers/final/status/')}}/"+'finalreject'+'/'+'{{$plots->farmer_uniqueId}}',
                 data: {_token:'{{csrf_token()}}',method:'post',plotno:plotno,reasons:reasons,rejectcomment:rejectcomment,},
                 success:function(data){
                     $('#Rspinner'+plotno).addClass('d-none');
                   location.reload();
