    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
         @php
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/pending/cropdata/plot/'.$prev->farmer_uniqueId}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>          
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('').'/'.$rolename.'/view/l2/pending/cropdata/plot/'.$next->farmer_uniqueId}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
          @endif
        </div>
    </div>
    <!-- row -->
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    ...
                    return '<div><a href="{{ url("/")}}/'+rolename+'/view/l2/pending/cropdata/plot/'+data.farmer_plot_uniqueid+'/'+data.plot_no+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$('.dt_irrigation_last').change(function() {
    ...
    var cropdata_end_days = '{{$date_setting->cropdata_end_days}}';
    var date_preparation = moment(irrigation_date, "DD-MM-YYYY").add(preparation_date_interval, 'days');
    var date_preparation_max = moment(date_preparation, "DD-MM-YYYY").add(cropdata_end_days, 'days');
    document.getElementById("dt_ploughing").setAttribute("min", moment(date_preparation).format('YYYY-MM-DD'));
    document.getElementById("dt_ploughing").setAttribute("max", moment(date_preparation_max).format('YYYY-MM-DD'));
...
$('.dt_ploughing').change(function() {
    ...
    var date_transplanting_max = moment(date_transplanting, "DD-MM-YYYY").add(cropdata_end_days, 'days');
    document.getElementById("dt_transplanting").setAttribute("min", moment(date_transplanting).format('YYYY-MM-DD'));
    document.getElementById("dt_transplanting").setAttribute("max", moment(date_transplanting_max).format('YYYY-MM-DD'));   
});
