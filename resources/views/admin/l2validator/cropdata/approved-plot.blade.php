            ...
            <div class=" bg-info countBox">
                <div class="card-body mini-stat-img boxm">
                    <div class="text-white">
                        ...
                        <h6 class="text-uppercase-mb-3 pin-font font-size-16 text-white text-center">Pending Plots
                        </h6>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="dropdown ml-auto">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item download-excel" data-type="CropData" href="{{url('l2/cropdata/download')}}">CropData</a>
                    </div> --}}
                        @endcan
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="card-body card-filter flex-container farmer-filter px-0">
                        <form class="row gx-3 gy-2 align-items-center form-filter" action="{{ url()->current() }}"
                            method="get">
                            <div class="col-sm-4">
                                <div class="input-daterange input-group" data-date-format="dd M, yyyy"
                                    data-date-autoclose="true" data-provide="datepickers">
                                    <input type="text" class="form-control start_date"
                                        value="{{ request()->query('start') }} " name="start" placeholder="To" />
                                    <input type="text" class="form-control end_date"
                                        value="{{ request()->query('end') }} " name="end" placeholder="From" />
                                </div>
                            </div>
                            <div class="col-sm-2">
                                <select id="states" onchange="FetchDistrict(this.value)" name="state"
                                    class="form-control select2">
                                    <option value="">States</option>
                                    @if ($states)
                                        @foreach ($states as $state)
                                            <option value="{!! $state->id !!}"
                                                {{ request()->query('state') == $state->id ? 'selected' : '' }}>
                                                {!! $state->name !!}</option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="col-sm-2">
                                <select id="districts" onchange="FetchBlock(this.value)" name="district"
                                    class="form-control select2">
                                    <option value="">Districts</option>
                                    @if ($districts)
                                        @foreach ($districts as $district)
                                            <option value="{!! $district->id !!}"
                                                {{ request()->query('district') == $district->id ? 'selected' : '' }}>
                            ...
                            <div class="col-sm-2">
                                <select id="talukas" onchange="FetchPanchayat(this.value)" name="taluka"
                                    class="form-control select2">
                                    <option value="">Taluka</option>
                                    @if ($talukas)
                                        @foreach ($talukas as $taluka)
                                            <option value="{!! $taluka->id !!}"
                                                {{ request()->query('taluka') == $taluka->id ? 'selected' : '' }}>
                                    ...
                                    @if ($panchayats)
                                        @foreach ($panchayats as $panchayat)
                                            <option value="{!! $panchayat->id !!}"
                                                {{ request()->query('panchayats') == $panchayat->id ? 'selected' : '' }}>
                                                {!! $panchayat->panchayat !!}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                                    ...
                                    @if ($villages)
                                        @foreach ($villages as $village)
                                            <option value="{!! $village->id !!}"
                                                {{ request()->query('village') == $village->id ? 'selected' : '' }}>
                            ...
                            <div class="col-sm-2 mt-2">
                                <select id="executive_onboarding" name="executive_onboarding"
                                    class="form-control select2">
                                    <option value="">Select Executive</option>
                                    @if ($onboarding_executive)
                                        @foreach ($onboarding_executive as $excutive)
                                            <option value="{{ $excutive->surveyor_id }}"
                                                {{ request()->query('executive_onboarding') == $excutive->surveyor_id ? 'selected' : '' }}>
                                                {{ $excutive->surveyor_name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="col-sm-2">
                                <button type="submit" class="btn btn-rounded btn-success"><span
                                        class="btn-icon-start text-dangers"></span>Submit</button>
                                <button type="button" class="btn btn-rounded btn-danger filter-remove"><span
                                        class="btn-icon-start text-dangers"><i class="fa fa-filter color-danger"></i>
                                    </span>Clear</button>
                            </div>
                        </form>
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="table table-bordered" style="min-width: 845px">
                            <!-- class="table table-bordered dt-responsive nowrap display data-table" -->
                            <thead>
                                <tr>
                                    <th>Sr.No</th>
                                    <th>Id</th>
                            ...
                            <tbody>
                                @if (!empty($crop_data) && $crop_data->count())
                                            ...
                                            <td>
                                                @if ($value->l2_status == 'Approved')
                                                    <a class="text-success"
                                                        title="Show">{{ $value->farmer_plot_uniqueid }}</a>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @php
                                                    $rolename =
                                                        auth()
                                                            ->user()
                                                            ->roles->first()->name == 'SuperAdmin'
                                                            ? 'admin'
                                                            : 'viewer';
                                                @endphp
                                                @if ($value->l2_status == 'Approved')
                                                    <a class="btn text-success"
                                                        href="{{ url('') }}/{{ $rolename }}/view/l2/approved/cropdata/plot/{{ $value->farmer_plot_uniqueid }}/{{ $value->plot_no }}"
                                                        title="Show"> {{ $value->plot_no }}</a>
                                                @endif
                                            </td>
                                            <td>
                                            @if ($value->plot_area==null || $value->plot_area=="0.00")
                                                {{ $value->area_acre_awd }}  
                                            @else
                                             {{$value->plot_area??'0.00'}}
                                            @endif
                                            </td> 
                                            <td>{{ $value->mobile }}</td>
                                            <td>{{ $value->state ?? 'NA' }}</td>
                                            <td>{{ $value->district ?? 'NA' }}</td>
                                            <td>{{ $value->taluka ?? 'NA' }}</td>
                                            <td>{{ $value->village ?? 'NA' }}</td>
                                            <td>{{$value->created_at??'NA'}}</td>
                                            <td>
                                                @if ($value->l2_status == 'Approved')
                        ...
                        <div class="pagination">
                            <ul class="pagination">
                                {{-- Previous Page --}}
                                <li class="page-item {{ $crop_data->currentPage() == 1 ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{$crop_data->previousPageUrl() }}" aria-label="Previous">
                                ...
                                {{-- First Page --}}
                                <li class="page-item">
                                    <a class="page-link" href="{{ $crop_data->url(1) }}">1</a>
                                </li>
                        
                                {{-- Pages in the middle --}}
                                @php
                                    $start = max(1, $crop_data->currentPage() - 2);
                                    $end = min($start + 4, $crop_data->lastPage());
                                @endphp
                        
                                @for ($i = $start; $i <= $end; $i++)
                                    <li class="page-item {{ $crop_data->currentPage() == $i ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $crop_data->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor
                        
                                {{-- Last Page --}}
                                <li class="page-item">
                                    <a class="page-link" href="{{ $crop_data->url($crop_data->lastPage()) }}">{{$crop_data->lastPage() }}</a>
                                </li>
                        
                                {{-- Next Page --}}
                                <li class="page-item {{ $crop_data->currentPage() == $crop_data->lastPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $crop_data->nextPageUrl() }}" aria-label="Next">
                        ...
                        <div class="pagination mt-3">
                            <p>
                                Showing {{ $crop_data->firstItem() }} to {{ $crop_data->lastItem() }} of {{ $crop_data->total() }} results
                            </p>
...
@section('scripts')
    ...
    <script>
        $(function() {
            let rolename = "{{ auth()->user()->roles->first()->name == 'SuperAdmin'? 'admin': 'viewer' }}";
            ...
            //     // Set the pagination type to "bootstrap"
            ...
            $.ajax({
                type: 'get',
                url: "{{ url('l2/fetch/counting') }}",
                dataType: 'Json',
                success: function(data) {
                    //   document.getElementById("FarmerCount").innerHTML = data.farmercount;
                    //   document.getElementById("PlotCount").innerHTML = data.plotcount;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.district, function(key, value) {
                            $('select[name="district"]').append('<option value="' + value.id + '">' +
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.Taluka, function(i, v) {
                            $('select[name="taluka"]').append('<option value="' + v.id + '">' + v
                                .taluka + '</option>');
                        });
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.panchayat, function(i, v) {
                            $('select[name="panchayats"]').append('<option value="' + v.id + '">' + v
                                .panchayat + '</option>');
                        });
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.Village, function(i, v) {
                            $('select[name="village"]').append('<option value="' + v.id + '">' + v
                                .village + '</option>');
                        });
                    }
                });
