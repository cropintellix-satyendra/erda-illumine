                ...
                <div class="dropdown ml-auto">
                    @can('Download Excel')
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item download-excel" data-type="onboarding" href="{{url('l2/download/file')}}">Farmer onboarding</a>
                        <a class="dropdown-item download-excel" data-type="cropdata" href="{{url('l2/download/file')}}">Crop data</a>
                        <a class="dropdown-item download-excel" data-type="PipeInstalltion" href="{{url('l2/download/file')}}">PipeInstallation</a>
                        <a class="dropdown-item download-excel" data-type="benefitsdata" href="{{url('l2/download/file')}}">Benefits</a>
                        <a class="dropdown-item download-excel" data-type="Aeration" href="{{url('l2/download/file')}}">Aerations</a>
                        <a class="dropdown-item download-excel" data-type="PipeGeojson" href="{{url('l2/download/file')}}">Pipe Geojson</a>
                    </div>
                    @endcan                  
                </div>
              </div>
                            ...
                            <form class="row gx-3 gy-2 align-items-center form-filter">
                                    ...
                                    <div class="col-sm-2 mt-2">
                                            <select id="module" name="module" class="form-control select2">
                                                    ...
                                                    <option value="PipeInstalltion">PipeInstalltion</option>
...
$(function(){
    ...
    var table=$('.data-table').DataTable({
        ...
        "columnDefs": [
            {render: function (data, type, row, meta) {
                @if(Auth::user()->hasRole('L-2-Validator'))
                    ...
                    if(row.final_status == 'Rejected'){
                      ...
                      return '<span class="'+status_color+'" title="Show">'+row.farmer_plot_uniqueid+'</span>';
                @elseif(Auth::user()->hasRole('SuperAdmin'))
                    ...
                    return '<a class="btn '+status_color+'" href="{{ url('l2/approved/show')}}/'+row.farmer_uniqueId+'" title="Show">'+row.farmer_plot_uniqueid+'</a>';
                @endif
                },
                "targets": 0,
            },
            {render: function (data, type, row, meta) {
                    ...
                    return '<a class="btn '+status_color+'" href="{{ url('l2/approved/plot/detail')}}/'+row.farmer_plot_uniqueid+'" title="Show">'+data+'</a>';
                },
                "targets": 2,
            },
            {render: function (data, type, row, meta) {
    ...
    $('.card-filter select').on('change',function(){
      ...
      $('.download-excel').on('click',function(e){
              ...
              var parms=$.param(clean({start_date,end_date,state,district,taluka,panchayats,village,farmer_status,executive_onboarding,file,type,status,modules,rolename,userid}));
              // if(type=='' || type==null){
