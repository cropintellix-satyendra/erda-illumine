    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                ...
                $prev=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','<',$Farmer->farmer_uniqueId)->where('final_status','Pending')->where('status','Approved')->orderBy('id','desc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                  return $q;
                })->first();
                $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->where('final_status','Pending')->where('status','Approved')->orderBy('id','asc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                  return $q;
                })->first();
            @endphp
    ...
    //search
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get("{!! url('l2/pending/search/') !!}", { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/pending/show')}}/'+data.id+'/'+data.farmer_uniqueId+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
