        ...
        <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">Pending Plots</h4>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                                    ...
                                    @if(!empty($plots_paginate) && $plots_paginate->count())
                                        @foreach($plots_paginate as $key => $value)
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- {{dd($value->final_status)}} --}}
                                                    @if($value->final_status == 'Pending')
                                                        <a class="text-info" title="Show">{{$value->farmer_plot_uniqueid}}</a>
                                                    @endif
                                                    @if($value->final_status == 'Rejected')
                                                        <a class="text-danger" title="Show">{{$value->farmer_plot_uniqueid}}</a>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    @if($value->final_status == 'Pending')
                                                     <a class="btn text-info" href="{{ url('admin/farmers/show')}}/{{$value->id}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                     @endif 
                                                     @if($value->final_status == 'Rejected')
                                                     <a class="btn text-danger" href="{{ url('admin/farmers/show')}}/{{$value->id}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                     @endif                                          
                                                </td>
                                                <td>{{ $value->area_in_acers }}</td>
                                                <td>{{ $value->plot_area }}</td>
                                                <td>{{ $value->mobile }}</td>
                                                <td>{{ $value->state->name }}</td>
                                                <td>{{ $value->district->district }}</td>
                                                <td>{{ $value->taluka->taluka??"-" }}</td>
                                                <td>{{ $value->village->village??"-" }}</td>
                                                ...
                                                {{-- <td>{{ $value->ApprvFarmerPlot->date_survey }}</td> --}}
                        ...
                        <div class="pagination">
                            <ul class="pagination">
                                {{-- Previous Page --}}
                                <li class="page-item {{ $plots_paginate->currentPage() == 1 ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $plots_paginate->appends(request()->query())->previousPageUrl() }}" aria-label="Previous">
                                ...
                                {{-- First Page --}}
                                <li class="page-item">
                                    <a class="page-link" href="{{ $plots_paginate->appends(request()->query())->url(1) }}">1</a>
                                </li>
                        
                                {{-- Pages in the middle --}}
                                @php
                                    $start = max(1, $plots_paginate->currentPage() - 2);
                                    $end = min($start + 4, $plots_paginate->lastPage());
                                @endphp
                        
                                @for ($i = $start; $i <= $end; $i++)
                                    <li class="page-item {{ $plots_paginate->currentPage() == $i ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $plots_paginate->appends(request()->query())->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor
                        
                                {{-- Last Page --}}
                                <li class="page-item">
                                    <a class="page-link" href="{{ $plots_paginate->appends(request()->query())->url($plots_paginate->lastPage()) }}">{{ $plots_paginate->lastPage() }}</a>
                                </li>
                        
                                {{-- Next Page --}}
                                <li class="page-item {{ $plots_paginate->currentPage() == $plots_paginate->lastPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $plots_paginate->appends(request()->query())->nextPageUrl() }}" aria-label="Next">
...
$(function(){
    ...
    //     "aaSorting": [[ 0, "desc" ]],
    //     "columns": [
    //         { "data": 'farmer_uniqueId',"name":'farmer_uniqueId','width':'5%',"searchable": true,},
    //         { "data": "farmer.farmer_name","name":"farmer.farmer_name","defaultContent":""},//'visible':false,'width':'0%'},
    //         { "data": "plot_no","name":"plot_no"},
    //         { "data": "area_in_acers","name":"area_in_acers"},
    //         { "data": "farmer.mobile","name":"farmer.mobile","defaultContent":""},
    //         { "data": "farmer.state","name":"farmer.state"},
    //         { "data": "farmer.district","name":"farmer.district"},
    //         { "data": "farmer.taluka","name":"farmer.taluka"},
    //         { "data": "farmer.village","name":"farmer.village"},
    ...
    //                         status_color='text-danger';
    //                     }
    //                     return '<a class="btn '+status_color+'" href="{{ url('admin/farmers/show')}}/'+row.id+'/'+row.farmer.farmer_uniqueId+'" title="Show">'+row.farmer_uniqueId+'</a>';
    //                 @endif
    //             },
    //             "targets": 0,
    //         },
    ...
    //                     status_color='text-danger';
    //                 }
    //                 return '<a class="btn '+status_color+'" href="{{ url('admin/farmer/final/plot')}}/'+row.id+'" title="Show">'+data+'</a>';
    //             },
    //             "targets": 2,
    //         },
    //         {render: function (data, type, row, meta) {
    ...
    //                     +' <a class="btn btn-sm btn-success waves-effect waves-light" href="{{ url('admin/farmer/plot')}}/'+row.id+'" data-toggle="tooltip" title="Show"><i class="ti-eye"></i></a>'
    //                    +'</div>';
    //                 @else
    //                    return '<div class="row-actions">'
    //                     +' <a class="btn btn-sm btn-success waves-effect waves-light" href="{{ url('admin/farmer/plot')}}/'+row.id+'" data-toggle="tooltip" title="Show"><i class="ti-eye"></i></a>'
    ...
    $('.card-filter select').on('change',function(){
      ...
      $('.download-excel').on('click',function(e){
              ...
              var file = 'excel';
              var status = '{{$status}}';
              var from = 'L2-Validator'
              var rolename = '{{\Auth::user()->roles->first()->name}}';
              var userid = '{{\Auth::user()->id}}';
              var l2_validator = $('select[name="l2_validator"]').val(); 

              var parms=$.param(clean({start_date,end_date,season,state,district,taluka,panchayats,village,farmer_status,executive_onboarding,file,type,status,from,rolename,userid,l2_validator}));
          ...
          $.ajax({
          type:'get',
          url: "{{url('admin/fetch/onboard/counting')}}",
          dataType: 'Json',
          success: function(data) {
