...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
if($status->final_status == 'Pending'  && $status->status == 'Pending'){
    $prev=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','<',$Farmer->farmer_uniqueId)->where('status','Pending')->orderBy('id','desc')->whereHas('farmer',function($q){
        $q->where('onboarding_form','1');
      return $q;
    })->first();

    $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->where('status','Pending')->orderBy('id','asc')->whereHas('farmer',function($q){
        $q->where('onboarding_form','1');
      return $q;
    })->first();

}elseif($status->final_status == 'Pending'  && $status->status == 'Rejected'){
    $prev=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','<',$Farmer->farmer_uniqueId)->where('status','Rejected')->orderBy('id','desc')->whereHas('farmer',function($q){
        $q->where('onboarding_form','1');
      return $q;
    })->first();

    $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->where('status','Rejected')->orderBy('id','asc')->whereHas('farmer',function($q){
        $q->where('onboarding_form','1');
      return $q;
    })->first();
}else{
        $prev=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','<',$Farmer->farmer_uniqueId)->where('final_status',$status->final_status)->where('status','Approved')->orderBy('id','desc')->whereHas('farmer',function($q){
                $q->where('onboarding_form','1');
              return $q;
            })->first();

            $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->where('final_status',$status->final_status)->where('status','Approved')->orderBy('id','asc')->whereHas('farmer',function($q){
                $q->where('onboarding_form','1');
              return $q;
            })->first();
}
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('admin/farmers/show',[$prev->id,$prev->farmer_uniqueId])}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('admin/farmers/show',[$next->id,$next->farmer_uniqueId])}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
