    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          ...
          $next=\App\Models\FarmerPlot::with('farmer')->where('farmer_uniqueId','>',$Farmer->farmer_uniqueId)->orderBy('id','asc')->whereHas('farmer',function($q){
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l1/all-farmer/show',[$prev->id,$prev->farmer_uniqueId])}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l1/all-farmer/show',[$next->id,$next->farmer_uniqueId])}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
