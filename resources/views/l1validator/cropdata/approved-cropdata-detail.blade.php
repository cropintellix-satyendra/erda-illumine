...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
            $prev=\App\Models\FarmerCropdata::select('id','farmer_plot_uniqueid','plot_no')->where('l2_status','Approved')->where('id','<',$crop_data_detail->id)->where('apprv_reject_user_id',auth()->user()->id)->orderBy('id','desc')->when(request(),function($q){

            return $q;
            })->first()??'';
            $next=\App\Models\FarmerCropdata::select('id','farmer_plot_uniqueid','plot_no')->where('l2_status','Approved')->where('id','>',$crop_data_detail->id)->where('apprv_reject_user_id',auth()->user()->id)->orderBy('id','asc')->when(request(),function($q){

            return $q;
            })->first()??'';
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l1/approved/cropdata/plot').'/'.$prev->farmer_plot_uniqueid.'/'.$prev->plot_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>          
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l1/approved/cropdata/plot').'/'.$next->farmer_plot_uniqueid.'/'.$next->plot_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
          @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/cropdata/'.'Individuals/'.$plot->farmer_plot_uniqueid.'/'.$plot->plot_no.'/'.'Approved')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>Crop Season</td>
                                                    <td>{{$Croplot->season}}</td>
                                                </tr>
                                                <tr>
                                                    <td>Crop Variety</td>
                                                    <td>{{$Croplot->crop_variety}}</td>
                                                </tr>
                                                <tr><td>Date of Irrigation last Season</td><td>{{ \Carbon\Carbon::createFromFormat('d/m/Y', $Croplot->dt_irrigation_last)->format('d-m-Y')}}</td></tr>
                                                <tr><td>Date of Land Preparation</td><td>{{ \Carbon\Carbon::createFromFormat('d/m/Y', $Croplot->dt_ploughing)->format('d-m-Y')}}</td></tr>
                                                <tr><td>Date of Transplanting</td><td>{{ \Carbon\Carbon::createFromFormat('d/m/Y', $Croplot->dt_transplanting)->format('d-m-Y')}}</td></tr>
                                                @if(Auth::user()->hasRole('SuperAdmin'))
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l1/cropdata/search/Approved') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/approved/cropdata/plot')}}/'+data.farmer_plot_uniqueid+'/'+data.plot_no+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
