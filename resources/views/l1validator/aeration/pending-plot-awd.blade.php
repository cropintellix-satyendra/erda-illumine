...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        @php
            $prev=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Pending')->where('id','<',$awd_data->id)->orderBy('id','desc')->when(request(),function($q){

            return $q;
            })->first()??'';

            $next=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Pending')->where('id','>',$awd_data->id)->orderBy('id','asc')->when(request(),function($q){

            return $q;
            })->first()??'';
        @endphp
        @if($prev)
          <a style="color: red;" href="{{ url('l1/pending/aeration/plot').'/'.$prev->farmer_plot_uniqueid.'/'.$prev->aeration_no.'/'.$prev->pipe_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
        @endif
        @if($next)
          <a style="color: red;" href="{{ url('l1/pending/aeration/plot').'/'.$next->farmer_plot_uniqueid.'/'.$next->aeration_no.'/'.$next->pipe_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
        @endif  
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                        <div class="col-6">
                                            <a class="btn text-success btn-sm" href="{{url('l1/download/aeration/'.'aeration'.'/'.$plot->farmer_plot_uniqueid.'/'.$awd_data->plot_no.'/'.'Pending/'.$awd_data->aeration_no)}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                        </div>
                                    @endcan
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row">
                              ...
                              <button style="width: 26%;"
                                  ...
                                  {{$awd_data->status == 'Rejected' ? 'Disabled' : ''}}{{$awd_data->status == 'Approved' ? 'Disabled' : ''}} >
                                    Approve
                                    <i class="fa fa-spinner fa-spin Aspinner d-none"></i>
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark" 
                                    class="btn btn-danger RejectBtn m-b-0 mr-3" {{$awd_data->status == 'Approved' ? 'Disabled' : ''}} {{$awd_data->status == 'Rejected' ? 'Disabled' : ''}}>
                                    Reject
                              </button>
                            </div>
                            @endif
                                            ...
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    ...
                                                    @foreach($validation_list  as $list)                                                      
                                                       <tr><td>{{$loop->index+1}}</td> <td>{{ $list->aeration_no}}</td><td>{{ $list->pipe_no}}</td> <td>{{ $list->status}}</td>  <td>{{ $list->comment}}</td>  <td>{{ $list->ValidatorUserDetail->name??""}}</td></tr>
                                                    @endforeach                                              
                                                </tbody>
                                            </table>
                                        </div>
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l1/aeration/search/Pending') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l1/pending/aeration/plot')}}/'+data.farmer_uniqueId+'/'+data.aeration_no+'/'+data.pipe_no+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
