...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                $prev=\App\Models\FarmerPlot::with('farmer')->where('id','<',$plot->id)->where('status','Pending')->orderBy('id','desc')->whereHas('farmer',function($q){
                ...
                $next=\App\Models\FarmerPlot::with('farmer')->where('id','>',$plot->id)->where('status','Pending')->orderBy('id','asc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                    if(auth()->user()->hasRole('L-1-Validator') && !request()->has('state')  && !request()->has('district') && !request()->has('taluka') && !request()->has('panchayats') && !request()->has('village')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l1/pending/plot/detail').'/'.$prev->id}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l1/pending/plot/detail').'/'.$next->id}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                                          ...
                                          <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    {{-- Storage::disk('s3')->url($items->path) --}}
                                                    @empty
                                                    @endforelse
                                                    </div>
                                                </td>
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Pending')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row mb-3">
                                    <a style="width: 30%;" href="{{url('l1/plot/edit/'.$plot->id.'/'.$plot->farmer_uniqueId)}}"
                                      class="btn btn-info m-b-0 mr-3 EditBtn {{ $plot->status == 'Approved' ? 'd-none' : '' }}">EDIT</a>
                                <!-- end button end -->
                              <button style="width: 26%;"
                                  data-toggle="modal" data-target="#ApproveModal"
                                  class="btn btn-success ApproveBtn m-b-0 mr-3" {{ $plot->status != 'Pending' ? 'disabled' : ''}} {{-- below code is to disable button if --}}
                                    >{{ $plot->status == 'Approved' ? 'Approved' : 'Approve'}}
                                    <i class="fa fa-spinner fa-spin Aspinner d-none"></i>
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#reject_remark" {{ $plot->status != 'Pending' ? 'disabled' : ''}}
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l1/pending/plot/detail/')}}/'+data.id+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
