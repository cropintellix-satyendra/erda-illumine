...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l1/pending/pipeinstallation/plot').'/'.$prev->farmer_plot_uniqueid}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a> 
          @endif                      
          @if($next)
          <a style="color: red;" href="{{ url('l1/pending/pipeinstallation/plot').'/'.$next->farmer_plot_uniqueid}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
          @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/pipeinstallation/'.'pipeinstallation'.'/'.$plot->farmer_plot_uniqueid.'/'.$PipeInstallation->plot_no.'/'.'Pending')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                      ...
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            <div class="row mb-3">
                              ...
                              <a style="width: 31%;" href="{{url('l2/pipeinstallation/pipeinstallation/plot/'.$plot->farmer_plot_uniqueid)}}" target="_blank" class="btn btn-status{{$plot->pipe_form ? '-done' : ' disabled'}} m-b-0 mr-3"><span class="btn-txt">Pipes Installations</span></a>
                            </div>
                            <div class="row mb-3">
                            ...
                            <div class="row">
                              ...
                              <button style="width: 26%;"
                                  data-toggle="modal" data-target="#PipeApprove"  {{ $PipeInstallation->status == 'Approved' ? 'disabled' : ''}} {{ $PipeInstallation->status == 'Rejected' ? 'disabled' : ''}}
                                  class="btn btn-success ApproveBtn m-b-0 mr-3">
                                    Approve
                                    <i class="fa fa-spinner fa-spin Aspinner d-none"></i>
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#Finalreject_remark"  {{ $PipeInstallation->status == 'Approved' ? 'disabled' : ''}} {{ $PipeInstallation->status == 'Rejected' ? 'disabled' : ''}}
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body text-center">                            
                              ...
                              <button class="btn info btn-info text-white UpdtePoly mt-2 {{ $PipeInstallation->status == 'Approved' ? 'd-none' : ''}} {{ $PipeInstallation->status == 'Rejected' ? 'd-none' : ''}}">Update Polygon</button>
                            @endif
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="margin-top: -53px;">
                <div class="container">
                  ...
                  <div class="row {{$pipeImage->status == 'Approved' ? 'd-none' : ''}}{{$pipeImage->status == 'Rejected' ? 'd-none' : ''}}">
                    <div class="col mt-2">
            ...
            <div class="modal-footer">
                ...
                <button type="button"  
                class="btn btn-primary SubmitApproval" Disableds>Save <i class="fa fa-spinner fa-spin Aspinner d-none"></i></button>
            </div>
            </form>
        </div>
    </div>
            ...
            <div class="modal-body">
              <div class="container">
                <div class="row">
                  ...
                  @foreach($PipesLocation as $pipeImage)
                          <div class="row {{$pipeImage->status == 'Rejected' ? 'd-none' : ''}} {{$pipeImage->status == 'Approved' ? 'd-none' : ''}}">
                            <div class="col">
                          ...
                          <div class="row {{$pipeImage->status == 'Rejected' ? 'd-none' : ''}} {{$pipeImage->status == 'Approved' ? 'd-none' : ''}}">
                                ...
                                <div class="col">
                                  <button {{$pipeImage->status == 'Rejected' ? 'd-none' : ''}} {{$pipeImage->status == 'Approved' ? 'd-none' : ''}}
                                           {{ $pipeImage->status == 'Rejected' ? 'disabled' : ''}} {{ $PipeInstallation->status =='Approved' ? 'Disabled' :'' }}
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l1/pipeinstallation/search/Pending') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l1/pending/pipeinstallation/plot')}}/'+data.farmer_uniqueId+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }                       
            }
        });
...
$(".PipeReject").click(function() {
    ...
    $('#Rspinner'+pipeno).removeClass('d-none');
        ...
        console.log(pipeno, pipe_id, reasons, rejectcomment);
