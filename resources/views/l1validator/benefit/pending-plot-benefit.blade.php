...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
              $prev=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Pending')->where('id','<',$benefit_data_detail->id)->orderBy('id','desc')->when(request(),function($q){

              return $q;
              })->first()??'';
              $next=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Pending')->where('id','>',$benefit_data_detail->id)->orderBy('id','asc')->when(request(),function($q){

              return $q;
              })->first()??'';
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l1/pending/benefit/plot').'/'.$prev->farmer_uniqueId}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l1/pending/benefit/plot').'/'.$next->farmer_uniqueId}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
          @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/benefit/'.'Individuals/'.$plot->farmer_uniqueId.'/'.'Pending')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row">
                              ...
                              <button style="width: 26%;"
                                  ...
                                  class="btn btn-success ApproveBtn m-b-0 mr-3 {{ $benefit_data_detail->status == 'Approved' ? 'disabled' : ''}}" {{-- below code is to disable button if --}}
                                    >
                                    Approve
                              </button>
                              <!-- approve end -->
    ...
    //search
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l1/benefit/search/Pending') !!}', { query: query }, function (data) {
                    var matches = [];
