...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                $prev=\App\Models\FarmerPlot::with('farmer')->where('id','<',$plot->id)->where('status','Rejected')->where('aprv_recj_userid',auth()->user()->id)->orderBy('id','desc')->whereHas('farmer',function($q){
                ...
                $next=\App\Models\FarmerPlot::with('farmer')->where('id','>',$plot->id)->where('aprv_recj_userid',auth()->user()->id)->where('status','Rejected')->orderBy('id','asc')->whereHas('farmer',function($q){
                    $q->where('onboarding_form','1');
                    if(auth()->user()->hasRole('L-1-Validator') && !request()->has('state')  && !request()->has('district') && !request()->has('taluka') && !request()->has('panchayats') && !request()->has('village')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l1/reject/plot/detail').'/'.$prev->id}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l1/reject/plot/detail').'/'.$next->id}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    @can('carbon download')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/plot/download/'.$plot->farmer_uniqueId.'/'.'CARBON'.'/'.'0')}}"><i class="fa fa-download" aria-hidden="true"></i> Carbon Consent</a>
                                    </div>
                                    @endcan
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Rejected')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                  ...
                  <div class="card" style="margin-right: -24px;margin-left: -24px;">
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row mb-3">
                                    <a style="width: 30%;" href="{{url('l1/plot/edit/'.$plot->id.'/'.$plot->farmer_uniqueId)}}"
                                      class="btn btn-info m-b-0 mr-3 EditBtn d-none">EDIT</a>
                                <!-- end button end -->
                              <button style="width: 26%;"
                                  data-toggle="modal" data-target="#ApproveModal" disabled
                                  class="btn btn-success ApproveBtn m-b-0 mr-3" {{ $plot->status != 'Rejected' ? 'disabled' : ''}} {{-- below code is to disable button if --}}
                                    >{{ $plot->status == 'Approved' ? 'Approved' : 'Approve'}}
                                    <i class="fa fa-spinner fa-spin Aspinner d-none"></i>
                              </button>
                              <!-- approve end -->
                              <button style="width: 30%;" data-toggle="modal" data-target="#reject_remark" {{ $plot->status != 'Rejected' ? 'disabled' : ''}}
                                    class="btn btn-danger RejectBtn m-b-0 mr-3"> {{ $plot->status == 'Rejected' ? 'Rejected' : 'Reject'}}</button>
                            </div>
                          </div><!-- button end -->
                      </div>
                  </div>
    ...
    'use strict';
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l1/plot/search/'.$plot->status) !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l1/reject/plot/detail')}}/'+data.id+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$(".SubmitApproval").click(function() {
    ...
    Swal.fire({
          ...
          if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('l1/status/')}}/"+'onboarding/{{$plot->farmer->farmer_uniqueId}}',
    ...
    if(!$('#plotno' + plotno).is(":checked")){
        ...
        Swal.fire({
          ...
          if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('l1/status/')}}/"+'reject'+'/'+'{{$plot->farmer->farmer_uniqueId}}',
