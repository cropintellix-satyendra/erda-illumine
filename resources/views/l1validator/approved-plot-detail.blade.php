        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
              if(auth()->user()->hasRole('L-1-Validator')){
                      $prev=\App\Models\FarmerPlot::with('farmer')->where('id','<',$plot->id)->where('status',$status)->orderBy('id','desc')->where('aprv_recj_userid',auth()->user()->id)->whereHas('farmer',function($q){
                      ...
                      $next=\App\Models\FarmerPlot::with('farmer')->where('id','>',$plot->id)->where('status',$status)->where('aprv_recj_userid',auth()->user()->id)->orderBy('id','asc')->whereHas('farmer',function($q){
                          $q->where('onboarding_form','1');
                              $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                              $q->whereIn('state_id',explode(',',$VendorLocation->state));
                              if(!empty($VendorLocation->district)){
                ...
                $prev=\App\Models\FarmerPlot::with('farmer')->where('id','<',$plot->id)->where('final_status',$status)->where('status','Approved')->orderBy('id','desc')->whereHas('farmer',function($q){
                            $q->where('onboarding_form','1');
                          return $q;
                        })->first();
            ...
            @endphp
            @if($prev)
            <a style="color: red;" href="{{ url('l1/approved/plot/detail').'/'.$prev->id}}"  class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i>Previous</a>
            @endif
            @if($next)
            <a style="color: red;" href="{{ url('l1/approved/plot/detail').'/'.$next->id}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
            @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Approved')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>    
                                    </div>
                                    @endcan
                                </div>
                            </div>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                @if(Auth::user()->hasRole('SuperAdmin'))
                                                    <a  target="_blank"  href="{{Route('admin.validator.edit',$plot->farmer->surveyor_id)}}">{{$plot->farmer->surveyor_name}}</a>
                                                @else
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row mb-3">
                              <button style="width: 26%;"
                                  ...
                                  class="btn btn-success ApproveBtn m-b-0 mr-3" {{ $plot->status != 'Approved' ? 'disabled' : ''}} {{-- below code is to disable button if --}}
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l1/approved/plot/detail/')}}/'+data.id+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
