                            ...
                            <form class="form-filter" action="{{url('admin/app/settings/update/')}}" method="post" enctype="multipart/form-data">
                              <div class="row">
                                <div class="col-md-6 col-xl-3 col-xxl-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <select id="organization" name="organization" class="form-control">
                                            ...
                                            <option value="{!! $organization->id !!}" data-name="{!! $organization->company !!}">{!! $organization->company !!}</option>
                                            @endforeach
                                            @endif
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-6 col-xl-3 col-xxl-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <select class="form-control date_wise" name="modules" id="modules" >
                                    ...
                                    <div id="TypeDownload" class="col-md-6 col-xl-3 col-xxl-6 mb-3  d-none">
                                        <label>Type </label>
                                        <div class="input-group">
                                            <select class="form-control" name="type_download" id="type_download">
...
{{auth()->user()->hasRole('L-2-Validator') ? '' : 'd-none'}}">
                                  ...
                                  <div id="report_type_div" class="col-md-6 col-xl-3 col-xxl-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <select class="form-control date-wise" name="report_type" id="report_type">
                                            <option value="">Select type</option>
                                ...
                                <div id="type_report_div" class="col-md-6 col-xl-3 col-xxl-6 mb-3 d-none">
                                    <label>Type of Report</label>
                                    <div class="input-group">
                                        <select class="form-control date-wise" name="type_report" id="type_report">
                                            <option value="">Select type</option>
                                            <option value="Farmer_wise">Farmer Wise</option>
                                            <option value="Plot_wise">Plot Wise</option>
...
<script>
    $(document).ready(function() {
        function changestatus() {
            ...
            var level = $('#level').val();
            var typeReport = $('#type_report').val(); // Get the value of type_report

            // Reset the visibility
            $('#report_type_div').addClass('d-none');
            $('#type_report_div').addClass('d-none');
            $('#aeration_no_container').addClass('d-none');
            $('#TypeDownload').addClass('d-none');

            if (modules) {
                if (modules === 'Polygon' || modules === 'CropData') {
                    // Hide both report_type_div and type_report_div
                    $('#report_type_div').addClass('d-none');
                    $('#type_report_div').addClass('d-none');
                } else if (modules === 'Onboarding' && level === 'All') {
                    // Show only report_type_div
                    $('#report_type_div').removeClass('d-none');
                } else if ((modules === 'PipeInstallation' || modules === 'Aeration') && level === 'All') {
                    // Show only type_report_div
                    $('#type_report_div').removeClass('d-none');
                } 
                
                // Show TypeDownload if modules is Polygon
                if (modules === 'Polygon' && level === 'All') {
                    $('#TypeDownload').removeClass('d-none');
                }

                // Show aeration_no_container if modules is Aeration, level is All, and typeReport is Plot_wise
                if (modules === 'Aeration' && level === 'All' && typeReport === 'Plot_wise') {
        ...
        // Bind change events
        $('#modules, #level, #type_report').change(function() {
            changestatus();
        });
    });
</script>
...
<script>
    $('.start_date').change(function() {
        ...
        $('.download-excel').on('click', function(e) {
            ...
            var organizationName = $('select[name="organization"] option:selected').data('name');
            ...
            var type_report = $('select[name="type_report"]').val();
