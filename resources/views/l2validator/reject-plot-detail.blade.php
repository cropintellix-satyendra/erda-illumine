...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            @php
                  $prev=\App\Models\FarmerPlot::with('farmer')->where('finalreject_userid',auth()->user()->id)->where('id','<',$plot->id)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','desc')
                  ...
                  ->first();
                  $next=\App\Models\FarmerPlot::with('farmer')->where('finalreject_userid',auth()->user()->id)->where('id','>',$plot->id)->where('final_status','Rejected')->where('status','Rejected')->orderBy('id','asc')
                  ->whereHas('farmerapproved',function($q){
                        $q->where('onboarding_form','1');
                        if(auth()->user()->hasRole('L-2-Validator')){
                            $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>Organization</td><td>{{$plot->organization->name??'NA'}}</td>
                                            </tr>
                                            <tr>
