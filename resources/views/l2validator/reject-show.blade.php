...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
              $prev=\App\Models\FinalFarmer::select('id','farmer_plot_uniqueid')->where('final_status_onboarding','Approved')->where('id','<',$plot->id)->orderBy('id','desc')
              ->whereHas('ApprvFarmerPlot',function($q){
              ...
              ->first()??'';
              $next=\App\Models\FinalFarmer::select('id','farmer_plot_uniqueid')->where('final_status_onboarding','Approved')->where('id','>',$plot->id)->orderBy('id','asc')
              ->whereHas('ApprvFarmerPlot',function($q){
                        $q->where('onboarding_form','1');
                        if(auth()->user()->hasRole('L-2-Validator')){
                            $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l2/approved/plot/detail').'/'.$prev->farmer_plot_uniqueid}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l2/approved/plot/detail').'/'.$next->farmer_plot_uniqueid}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
          @endif
        </div>
    </div>
    <!-- row -->
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>Guardian Name</td><td>{{$farmer->guardian_name??'NA'}}</td>
                                            </tr>
                                            <tr>
                                                <td>Aadhaar Number</td><td>{{$farmer->aadhaar??'NA'}}</td>
                                            </tr>
                                            <tr>
                                            <tr>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    ...
                                                    <tr><td>Area In Hectare</td><td>{{$Croplot->area_in_acers}}</td></tr>
                                                    {{-- <tr><td>Area Chosen For AWD(Bigha)</td><td>{{$plot->ApprvFarmerPlot->area_other_awd}}</td></tr> --}}
                                                    <tr><td>Area Chosen For AWD(Hectare)</td><td>{{$plot->ApprvFarmerPlot->area_acre_awd}}</td></tr>
                                                    <tr><td>Owner Name</td><td>{{$plot->ApprvFarmerPlot->actual_owner??"NA"}}</td></tr>
                                                    @if ($farmer->state_id == 29)
                                                        <tr><td>Patta Number</td><td>{{$plot->patta_number}}</td></tr>
                                                        <tr><td>Daag Number</td><td>{{$plot->daag_number}}</td></tr>
                                                    @endif
                                                    @if ($farmer->state_id == 37)
                                                        <tr><td>Khatian Number</td><td>{{$plot->khatian_number}}</td></tr>
                                                    @endif
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                        <a class="btn text-success btn-sm" href="{{url('l2/download/file/'.'Individuals/'.$plot->farmer_uniqueId.'/'.$plot->plot_no.'/'.'Approved')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                                        </td>
                                                    </tr>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>Mobile No</td><td>{{$plot->users->mobile}}</td>
                                            </tr>
                                            <tr>
                                                <td>Email ID</td><td>{{$plot->users->email}}</td>
                                            </tr>
                                            <tr>
                      ...
                      <div class="card-body" style="padding-left: 23px;padding-top: 11px;padding-right: 2px;">
                        ...
                        <div class="mb-1">
                            <div class="row mb-3">
                              ...
                              <a style="width: 31%;" href="{{url('l2/pipeinstallation/plot/'.$plot->farmer_plot_uniqueid)}}" target="_blank" class="btn btn-status{{$check_pipedata ? '-done' : ' disabled'}} m-b-0 mr-3"><span class="btn-txt">Polygon</span></a>
                            </div>
                            <div class="row mb-3">
        ...
        $('input[name="search"]').typeahead({
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/approved/plot/detail')}}/'+data.farmer_plot_uniqueid+'"><strong>' + data.farmer_plot_uniqueid + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
