...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
          @php
              $prev=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Approved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','<',$benefit_data_detail->id)->orderBy('id','desc')
              ...
              ->first()??'';
              $next=\App\Models\FarmerBenefit::select('id','farmer_uniqueId')->where('status','Approved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','>',$benefit_data_detail->id)->orderBy('id','asc')
              ->whereHas('farmerapproved',function($q){
                    if(auth()->user()->hasRole('L-2-Validator')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l2/approved/benefit/plot').'/'.$prev->farmer_uniqueId}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l2/approved/benefit/plot').'/'.$next->farmer_uniqueId}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
          @endif
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="tab-content">
                                    {{-- @foreach($plot as $Croplot) --}}
                                    <div id="plot-{{$plot->plot_no}}" class="tab-pane active pt-2">
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    <tr><td>Plot Id</td><td>{{$plot->farmer_plot_uniqueid}}</td></tr>
                                                    <tr><td>Area In Bigha</td><td>{{$plot->ApprvFarmerPlot->area_in_other}}</td></tr>
                                                    <tr><td>Area In Acres</td><td>{{$plot->area_in_acers}}</td></tr>
                                                    <tr><td>Area Chosen For AWD(Bigha)</td><td>{{$plot->ApprvFarmerPlot->area_other_awd}}</td></tr>
                                                    <tr><td>Area Chosen For AWD(Acres)</td><td>{{$plot->ApprvFarmerPlot->area_acre_awd}}</td></tr>
                                                    <tr><td>Owner Name</td><td>{{$plot->actual_owner_name}}</td></tr>
                                                    @if ($plot->state_id == 29)
                                                    <tr><td>Patta Number</td><td>{{$farmerplot->patta_number??""}}</td></tr>
                                                    <tr><td>Daag Number</td><td>{{$farmerplot->daag_number??""}}</td></tr>
                                                    @endif
                                                    @if ($plot->state_id == 37)
                                                    <tr><td>Patta Number</td><td>{{$farmerplot->ApprvFarmerPlot->patta_number??""}}</td></tr>
                                                    <tr><td>Daag Number</td><td>{{$farmerplot->ApprvFarmerPlot->daag_number??""}}</td></tr>
                                                    @endif
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l2/download/benefit/'.'Individuals/'.$plot->farmer_uniqueId.'/'.'Approved')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            ...
                            <div class="row">
                              ...
                              <button style="width: 26%;"
                                  ...
                                  class="btn btn-success ApproveBtn m-b-0 mr-3 {{ $benefit_data_detail->status == 'Approved' ? 'disabled' : ''}} d-none" {{-- below code is to disable button if --}}
                                    >
                                    Approve
                              </button>
                              <!-- approve end -->
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
                ...
                <div class="container">
                  ...
                  <div class="row">
                    <div class="col mt-2">
                      ...
                      {{ $benefit_data_detail->status == 'Rejected' ? 'disabled' : ''}}
                      {{ $benefit_data_detail->status == 'Approved' ? 'checked disabled' : ''}}
                      name="onboarding" value="{{$benefit_data_detail->farmer_uniqueId}}" {{$benefit_data_detail->status == 'Approved' ?' ':''}}>
                      <label title="{{ $benefit_data_detail->status == 'Rejected' ? 'Rejected' : ''}}{{ $benefit_data_detail->status == 'Approved' ? 'Approved' : ''}}{{ $benefit_data_detail->status == 'Pending' ? 'Pending' : ''}}"
                          for="onboarding"
                      ...
                      <div style="margin: 0px 0px 0px 17px;">
                            ...
                            <textarea {{ $benefit_data_detail->status == 'Approved' ? 'readonly' : ''}}  {{ $benefit_data_detail->status == 'Rejected' ? 'disabled' : ''}}
            ...
            <div class="modal-footer">
                ...
                <button type="button" {{ $benefit_data_detail->status == 'Approved' ? 'disabled' : ''}}
    ...
    //search
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/benefit/search/Approved') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/approved/benefit/plot')}}/'+data.farmer_uniqueId+'"><strong>' + data.farmer_uniqueId	 + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
...
$(".SubmitApproval").click(function() {
    ...
    Swal.fire({
		  ...
		  if (result.value == 1) {
                ...
                $.ajax({
                  type:'post',
                  url:"{{url('l1/benefit/status/')}}/"+'{{$benefit_data_detail->farmer_uniqueId}}',
