                                        ...
                                        @foreach($plots_paginate as $key => $value)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if($value->l2_status == 'Pending')
                                                     <a class="btn text-info" href="{{ url('l2/pending/cropdata/plot')}}/{{$value->farmer_plot_uniqueid}}" title="Show"> {{ $value->plot_no }}</a>
                                                     @endif                                                   
                                                </td>
                                                <td>{{ $value->farmerapproved->area_in_acers }}</td>
...
$(function(){
    ...
    //                   }
    //                   return '<input type="checkbox" class="farmeridcheckbox" name="id["'+row.farmer_plot_uniqueid+'"]" value="'+row.farmer_plot_uniqueid+'">';
    ...
    //                     status_color='text-danger';
    //                 }
    //                 return '<a class="btn '+status_color+'" href="{{ url('l2/pending/cropdata/plot')}}/'+row.farmer_plot_uniqueid+'/'+row.plot_no+'" title="Show">'+data+'</a>';
    //             },
    //             "targets": 3,
    //         },
    //         {render: function (data, type, row, meta) {
   ...
   $('.approve-record').on('click', function(e){    
        farmer_unique_id_array=[];
        const checkboxes = document.querySelectorAll('input[class="farmeridcheckbox"]');
        for (var i = 0; i < checkboxes.length; i++) {
            var checkbox = checkboxes[i];
                if (checkbox.checked){                    
                    var farmer_unique_id = {
                        farmer_unique_id: checkbox.value
                    };
                    farmer_unique_id_array.push(farmer_unique_id);                   
                }         
        }

        if(farmer_unique_id_array.length == 0){
        ...
        Swal.fire({
		  ...
		  if (result.value == 1) {
            $("#appr-btn").addClass('d-none');
            $("#appr-btn-spinner").removeClass('d-none');
            console.log(farmer_unique_id_array);
                $.ajax({
                  type:'post',
                  url:"{{url('l2/cropdata/bulk/approval')}}",
                  data: {_token:'{{csrf_token()}}',method:'post',farmer_unique_id_array:farmer_unique_id_array,},
                  ...
                  error: function (jqXHR, textStatus, errorThrown) {
                    $("#appr-btn").removeClass('d-none');
                    $("#appr-btn-spinner").addClass('d-none');
...
//    });
    ...
    //   });
      ...
      $('.download-excel').on('click',function(e){
              ...
              var parms=$.param(clean({start_date,end_date,season,state,district,taluka,panchayats,village,farmer_status,executive_onboarding,file,type,status,rolename}));
              // if(type=='' || type==null){
