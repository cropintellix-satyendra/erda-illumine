...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        @php
              $prev=\App\Models\FarmerCropdata::select('id','farmer_plot_uniqueid','plot_no')->where('status','Approved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','<',$crop_data_detail->id)->orderBy('id','desc')
            ...
            return $q;
              ...
              ->first()??'';
              $next=\App\Models\FarmerCropdata::select('id','farmer_plot_uniqueid','plot_no')->where('status','Approved')->where('l2_status','Approved')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','>',$crop_data_detail->id)->orderBy('id','asc')
              ->whereHas('farmerapproved',function($q){
                if(auth()->user()->hasRole('L-2-Validator')){
                    $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                    $q->whereIn('state_id',explode(',',$VendorLocation->state));
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l2/approved/cropdata/plot').'/'.$prev->farmer_plot_uniqueid.'/'.$prev->plot_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l2/approved/cropdata/plot').'/'.$next->farmer_plot_uniqueid.'/'.$next->plot_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
          @endif
        </div>
    </div>
    <!-- row -->
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/cropdata/search/Approved') !!}', { query: query }, function (data) {
                    var matches = [];
