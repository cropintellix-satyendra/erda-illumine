    ...
    <div class="row page-titles mx-0">
        <div class="col-sm-3 p-md-0">
            <div class="welcome-text">
                <h4>Farmer Plot Details dfgdg</h4>
            </div>
        </div>
        <div class="col-md-3">
        ...
        @endphp
        @if($prev)
          <a style="color: red;" href="{{ url('l2/pending/aeration/plot').'/'.$prev->farmer_plot_uniqueid.'/'.$prev->aeration_no.'/'.$prev->pipe_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
        @endif
        @if($next)
          <a style="color: red;" href="{{ url('l2/pending/aeration/plot').'/'.$next->farmer_plot_uniqueid.'/'.$next->aeration_no.'/'.$next->pipe_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
        @endif
        </div>
    </div>
    <!-- row -->
        ...
        <div class="col-md-4">
            ...
            <div class="col-12">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                           @include('farmer_detail_extends.awd_event')
                        </div>
                    </div>
                </div>
                @endif

                <!--PipeInstallation data-->
                                    ...
                                    <div id="plot-{{$PipeInstallation->plot_no}}" class="tab-pane active pt-2">
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    <tr><td>Level</td> <td>Aeration no</td> <td>Pipe no</td> <td>Status</td>  <td>Comment</td> <td>L2-Validate</td> <td>Validate On</td></tr>
                                                    @foreach($validation_list  as $list)
                                                       <tr><td>{{$list->level}}</td> <td>{{ $list->aeration_no}}</td><td>{{ $list->pipe_no}}</td> <td>{{ $list->status}}</td>  <td>{{ $list->comment}}</td>  <td>{{ $list->ValidatorUserDetail->name??""}}</td> <td> {{ $list->created_at??""}}</td></tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                                        </div>
                  ...
                  <div class="row">
                    <div class="col mt-2">
                      ...
                      <label  title="{{ $items->status == 'Rejected' ? 'Rejected' : ''}}{{ $items->status == 'Approved' ? 'Approved' : ''}}{{ $items->status == 'Pending' ? 'Pending' : ''}}"
                          for="pipe_no"
                          style="margin-right: 11px;">Aeration no {{$items->aeration_no}}</label>&nbsp;
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/aeration/search/Pending') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/pending/aeration/plot')}}/'+data.farmer_uniqueId+'/'+data.aeration_no+'/'+data.pipe_no+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
