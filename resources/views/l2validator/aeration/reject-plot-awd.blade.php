...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        @php
            $prev=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Rejected')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','<',$awd_data->id)->orderBy('id','desc')
            ...
            $next=\App\Models\Aeration::select('id','farmer_plot_uniqueid','aeration_no','pipe_no')->where('pipe_no',$awd_data->pipe_no)->where('status','Approved')->where('l2_status','Rejected')->where('l2_apprv_reject_user_id',auth()->user()->id)->where('id','>',$awd_data->id)->orderBy('id','asc')
            ->whereHas('farmerapproved',function($q){
                    if(auth()->user()->hasRole('L-2-Validator')){
                        $VendorLocation = DB::table('vendor_locations')->where('user_id',auth()->user()->id)->first();
                        $q->whereIn('state_id',explode(',',$VendorLocation->state));
        ...
        @endphp
        @if($prev)
          <a style="color: red;" href="{{ url('l2/rejected/aeration/plot').'/'.$prev->farmer_plot_uniqueid.'/'.$prev->aeration_no.'/'.$prev->pipe_no}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
        @endif
        @if($next)
          <a style="color: red;" href="{{ url('l2/rejected/aeration/plot').'/'.$next->farmer_plot_uniqueid.'/'.$next->aeration_no.'/'.$next->pipe_no}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a> 
        @endif  
        </div>
    </div>
    <!-- row -->
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l2/download/aeration/'.'aeration'.'/'.$plot->farmer_plot_uniqueid.'/'.$awd_data->plot_no.'/'.'Rejected/'.$awd_data->aeration_no)}}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/aeration/search/Rejected') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/rejected/aeration/plot')}}/'+data.farmer_uniqueId+'/'+data.aeration_no+'/'+data.pipe_no+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
