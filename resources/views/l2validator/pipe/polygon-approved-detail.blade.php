...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        ...
        @php
          ...
          @endphp
          @if($prev)
          <a style="color: red;" href="{{ url('l2/approved/pipeinstallation/plot').'/'.$prev->farmer_plot_uniqueid}}" class="btn btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous</a>
          @endif
          @if($next)
          <a style="color: red;" href="{{ url('l2/approved/pipeinstallation/plot').'/'.$next->farmer_plot_uniqueid}}" class="btn btn-sm">Next <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
          @endif
        </div>
    </div>
    <!-- row -->
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm">
                                                <tbody>
                                                    ...
                                                    <tr><td>Area In Acres</td><td>{{$plot->ApprvFarmerPlot->area_in_acers}}</td></tr>
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer p-0">
                                <div class="row">
                                    ...
                                    @can('Download Excel')
                                    <div class="col-6">
                                        <a class="btn text-success btn-sm" href="{{url('l1/download/pipeinstallation/'.'pipeinstallation'.'/'.$plot->farmer_plot_uniqueid.'/'.$PipeInstallation->plot_no.'/'.'Approved')}}"><i class="fa fa-download" aria-hidden="true"></i> Download Excel</a>
                                    </div>
                                    @endcan
                                </div>
                            </div>
...
<script src="https://maps.googleapis.com/maps/api/js?key={{config('map.map_key')}}&libraries=geometry,places,drawing&amp;ext=.js"></script>
...
function addMeasureControls(map){
    ...
    controlContainer.style.background = '#fff';
    controlContainer.style.border = '1px solid #ccc';
    controlContainer.style.padding = '6px 8px';
    ...
    toggleBtn.textContent = 'Measure distance';
    ...
    distanceLabel.textContent = 'Distance: 0 m';
    distanceLabel.style.fontWeight = '600';
    distanceLabel.style.marginLeft = '4px';
    ...
    function computeAndDisplayDistance(polyline){
        var lengthMeters = google.maps.geometry.spherical.computeLength(polyline.getPath());
        distanceLabel.textContent = 'Distance: ' + formatDistance(lengthMeters);
    }
    ...
    google.maps.event.addListener(drawingManager, 'polylinecomplete', function(polyline) {
        ...
        measureActive = false;
        toggleBtn.textContent = 'Measure distance';
        computeAndDisplayDistance(polyline);
        google.maps.event.addListener(polyline.getPath(), 'set_at', function(){ computeAndDisplayDistance(polyline); });
        google.maps.event.addListener(polyline.getPath(), 'insert_at', function(){ computeAndDisplayDistance(polyline); });
    });
    ...
    toggleBtn.addEventListener('click', function(){
        if(measureActive){
            ...
            toggleBtn.textContent = 'Finish measuring';
        }
    });
    ...
    //search
        ...
        $('input[name="search"]').typeahead({
            ...
            source:function (query, process) {
                return $.get('{!! url('l2/pipeinstallation/search/Approved') !!}', { query: query }, function (data) {
                    var matches = [];
            ...
            templates: {
                suggestion: function(data) {
                    return '<div><a href="{{ url('l2/approved/pipeinstallation/plot')}}/'+data.farmer_uniqueId+'"><strong>' + data.farmer_uniqueId + '</strong> - ' + data.status + '</a></div>';
                }
            }
        });
